...
function renderMainNavigation(block, appItems, activePageKey = "") {
  ...
  return [
    ...
    `  <div class="rs-nav-inner">`,
    `    <a class="rs-logo" href="/rs/home" data-rs-page-key="rs_home" aria-label="RingStatus home">RING<span>STATUS</span></a>`,
    `    <div class="rs-nav-links">`,
...
function renderBaseStyle() {
  return [
    ...
    `.rs-nav-inner{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:center;gap:12px;width:min(100%,1520px);margin:0 auto;padding:14px 16px;}`,
    `.rs-logo{display:inline-flex;align-items:center;justify-content:flex-start;color:#fff;text-decoration:none;font:800 24px/1 Outfit,Arial,sans-serif;letter-spacing:-.04em;white-space:nowrap;}`,
    `.rs-logo span{color:#56372d;}`,
    `.rs-nav-links{display:flex;align-items:center;justify-content:flex-start;gap:10px;min-width:0;overflow-x:auto;scrollbar-width:none;}`,
    `.rs-nav-links::-webkit-scrollbar{display:none;}`,
    `.rs-nav-button,.rs-nav-link{display:inline-flex;align-items:center;justify-content:center;flex:0 0 auto;min-height:42px;border:1px solid #d8dee6;border-radius:999px;padding:0 18px;background:#fff;color:#10243b;text-decoration:none;font:600 12px/1 Outfit,Arial,sans-serif;text-transform:uppercase;letter-spacing:.08em;white-space:nowrap;box-shadow:0 2px 10px rgba(15,23,42,.18);cursor:pointer;}`,
    ...
    `@media(min-width:480px){.rs-nav-inner{padding:16px 20px;gap:14px}.rs-logo{font-size:28px}.rs-nav-button,.rs-nav-link{padding:0 20px}}`,
    `@media(min-width:768px){.rs-nav-inner{display:flex;justify-content:space-between;padding:20px 32px;gap:24px}.rs-logo{font-size:32px}.rs-nav-links{justify-content:flex-end;gap:14px}.rs-nav-button,.rs-nav-link{min-height:44px;padding:0 22px;font-size:13px}}`,
    `@media(min-width:1024px){.rs-nav-inner{padding:22px 40px}}`,
    `@media(max-width:700px){.rs-mega-inner{display:grid;grid-template-columns:1fr;gap:20px;padding:24px 16px}.rs-drawer{width:100vw}.rs-content-flex{display:flex;flex-direction:column;align-items:stretch;gap:20px}.rs-visual{width:100%;flex:auto;aspect-ratio:16/10;min-height:0}}`,
