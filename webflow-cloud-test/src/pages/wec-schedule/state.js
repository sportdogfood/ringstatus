...
export const GET = async ({ request }) => {
  ...
  try {
    ...
    const rows = classRows
      ...
      .filter((fields) => clean(fields.show_no) === showNo)
      ...
      .map((fields) => classRowToScheduleRow({
        fields,
        focus,
        focusDay,
        ringDisplayByNo,
    ...
    const entryOnlyClassRows = entryRows
      .map((record) => record.fields || {})
      .filter((fields) => clean(fields.show_no) === showNo)
      .filter((fields) => isoDate(fields.focus_day) === focusDay)
      .filter((fields) => normalizeStatus(fields.status) !== "inactive")
      .filter((fields) => clean(fields.class_no) && !rowClassNos.has(clean(fields.class_no)))
      .filter((fields, index, all) => all.findIndex((item) => clean(item.class_no) === clean(fields.class_no)) === index)
      .filter((fields) => !isHiddenClass(fields, hide))
      .map((fields) => classRowToScheduleRow({
        fields,
        focus,
...
function getAirtableConfig() {
  ...
  const baseId = runtime.WEC_AIRTABLE_BASE_ID || runtime.AIRTABLE_WEC_SCHEDULES_BASE_ID || DEFAULT_BASE_ID || runtime.AIRTABLE_BASE_ID || runtime.AIRTABLE_BASE;
...
async function getFocusShow(airtable, showNo) {
  ...
  const record = records.find((item) => item.fields?.active && clean(item.fields?.show_no) === showNo)
    || records.find((item) => clean(item.fields?.show_no) === showNo)
...
function groupEntries(records, showNo, focusDay) {
  ...
  for (const record of records || []) {
    ...
    if (!trainer) continue;
    const classMap = byClassTrainer.get(classNo) || new Map();
...
function compareScheduleRows(a, b) {
  return clean(a.ring_name).localeCompare(clean(b.ring_name))
