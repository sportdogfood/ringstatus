...
async function buildSection({ token, baseId, pageKey, blockKey }) {
  ...
  const block = blockKey
    ? pageBlocks.find((record) => clean(record.fields.block_key) === blockKey)
    : pageBlocks.find((record) => clean(record.fields.block_type) !== "navigation") || pageBlocks[0];
  ...
  const blockDivs = divs
    .filter((record) => includes(record.fields.block, block.id))
    .sort(sortByOrder);
  const typographyByDiv = new Map();
  ...
  const contentByTypography = new Map();
...
function sortByOrder(a, b) {
  return Number(a.fields?.sort_order || 0) - Number(b.fields?.sort_order || 0) ||
    clean(a.fields?.block_key || a.fields?.div_key || a.fields?.typography_key || a.fields?.content_key)
      .localeCompare(clean(b.fields?.block_key || b.fields?.div_key || b.fields?.typography_key || b.fields?.content_key));
}
