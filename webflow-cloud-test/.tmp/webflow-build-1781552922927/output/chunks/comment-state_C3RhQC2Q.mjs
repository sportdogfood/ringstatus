...
const GET = async ({ request }) => {
  ...
  try {
    ...
    const classItems = classRows.map((record) => record.fields || {}).filter((fields) => clean(fields.show_no).replace(/\.0$/, "") === showNo).filter((fields) => isoDate(fields.focus_day) === focusDay).filter((fields) => normalizeStatus(fields.status) !== "inactive").filter((fields) => !isHiddenClass(fields, hide)).map((fields) => classRowToCommentClass(fields, focus, focusDay, rings, entriesByClass)).sort(compareClasses);
...
async function getFocusShow(airtable, showNo) {
  ...
  const record = records.find((item) => item.fields?.active && clean(item.fields?.show_no).replace(/\.0$/, "") === showNo) || records.find((item) => clean(item.fields?.show_no).replace(/\.0$/, "") === showNo) || records.find((item) => item.fields?.active) || records[0];
...
function classRowToCommentClass(fields, focus, focusDay, rings, entriesByClass) {
  const classNo = clean(fields.class_no).replace(/\.0$/, "");
  const ringNo = clean(fields.ring_no).replace(/\.0$/, "");
  const ringInfo = rings.get(ringNo) || { ring_name: clean(fields.ring_display || fields.ring_name) || `Ring ${ringNo}`, priority: 0 };
...
function groupClassesByRing(classItems, rings) {
  ...
  return Array.from(byRing.values()).map((ring) => ({
    ...ring,
...
function compareClasses(a, b) {
  return a.ring_priority - b.ring_priority || clean(a.ring_name).localeCompare(clean(b.ring_name)) || clean(a.class_start_time).localeCompare(clean(b.class_start_time)) || numberOrZero(a.class_number) - numberOrZero(b.class_number);
}
