...
function buildPageFromDataset({ baseId, pageKey, dataset }) {
  ...
  const tree = {
    ...
    navigation: navigation.filter((record) => selectName(record.fields.active) === "active").sort(sortByOrder).map((record) => pick(record, ["nav_item_key", "nav_group", "label", "page_key", "href", "sort_order"])),
    repeatableItemTypes: repeatableItemTypes.filter((record) => selectName(record.fields.active) === "active").sort(sortByOrder).map((record) => pick(record, ["item_type_key", "item_type_label", "parent_slot_role", "default_class"])),
    blocks: treeBlocks
  };
...
function getMainNavPageKeys(items) {
  return [...new Set(items.filter((item) => selectName(item.nav_group) === "main_nav" || selectName(item.nav_group) === "app_nav").sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0)).map((item) => clean(item.page_key)).filter(Boolean))];
}
...
function isRepeatableBlock(block, repeatableItemTypes = []) {
  ...
  return htmlKey === "sticky_scroll_cards" || repeatableItemTypes.some((item) => clean(item.item_type_key) === componentKey);
}
...
function renderNavigation(block, items, group, activePageKey = "") {
  const appItems = items.filter((item) => selectName(item.nav_group) === "app_nav").sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0));
...
function renderMainNavigation(block, appItems, activePageKey = "") {
  ...
  const appColumns = columns.filter((column) => column.items.length).map((column) => [
...
function renderRepeatableSection(block) {
  const cards = block.divs.flatMap((div) => div.typography).flatMap((typeRow) => typeRow.content).sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0)).filter((contentRow) => selectName(contentRow.active) !== "inactive");
