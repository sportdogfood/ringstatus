...
async function publish(request) {
  ...
  try {
    const rendered = await fetchRenderedPage({ request, pageKey });
...
async function fetchRenderedPage({ request, pageKey }) {
  const renderUrl = new URL("/test/rs-page-render", request.url);
  renderUrl.searchParams.set("pageKey", pageKey);
  renderUrl.searchParams.set("refresh", "1");
  renderUrl.searchParams.set("_", String(Date.now()));
  const response = await fetch(renderUrl, { cache: "no-store" });
  const payload = await response.json();
  if (!response.ok || !payload?.ok) {
    throw new Error(payload?.detail || payload?.error || `render_failed:${response.status}`);
  }
  return payload;
}
