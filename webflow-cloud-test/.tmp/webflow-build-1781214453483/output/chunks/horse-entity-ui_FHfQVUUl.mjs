...
const HORSE_ENTITY_ALLOWED_WRITE_FIELDS = [
  "horse",
  "barn_name",
  "show_name",
  "active",
...
const HORSE_ENTITY_ALLOWED_CREATE_FIELDS = HORSE_ENTITY_ALLOWED_WRITE_FIELDS;
...
function runtimeEnv(extra = {}) {
  ...
  const astroEnv = Object.assign(__vite_import_meta_env__, { AIRTABLE_WEC_META_TABLE: "tbllJywsOstkqT5yZ", AIRTABLE_BASE_ID: "apptdhhNzduxm5gjn", AIRTABLE_TOKEN: "patDeqY9NAQsuYx6q.7fd75026f0820373f62a72ca063f99b2203b9d873cb77aa3962637ab7bb0ec37", COLOR: "0", OS: "Windows_NT" }) || {};
...
async function horseEntityReport(airtable, requestUrl, adapter = createAirtableAdapter(airtable)) {
...
async function horseEntityActionReport(airtable, requestUrl, payload, adapter = createAirtableAdapter(airtable)) {
...
function createAirtableAdapter(airtable) {
...
async function airtableFetch(url, options, errorPrefix) {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    const delay = Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1e3 : 350 * (attempt + 1);
    await sleep(delay);
  }
...
export {
  airtableConfig as a,
  horseEntityActionReport as b,
  corsHeaders as c,
  horseEntityReport as h,
  json as j,
  runtimeEnv as r
};
