...
async function addComment(airtable, schema, payload) {
  ...
  const scopedTable = commentScope === "ring" ? airtable.ringCommentsTable : commentScope === "class" ? airtable.classCommentsTable : airtable.entryCommentsTable;
...
async function listRingCheckins(airtable, payload) {
  ...
  const checkins = records.map((record) => ({ record_id: record.id, ...record.fields || {} })).filter((fields) => !sessionId || clean(fields.session_id) === sessionId).filter((fields) => !showNo || clean(fields.show_no).replace(/\.0$/, "") === showNo).filter((fields) => !focusDay || isoDate(fields.focus_day) === focusDay).filter((fields) => clean(fields.status).toLowerCase() === "active").filter((fields) => {
    const seen = Date.parse(clean(fields.last_seen_at || fields.checked_in_at));
    return Number.isFinite(seen) && seen >= cutoff;
  }).sort((a, b) => String(b.last_seen_at || b.checked_in_at).localeCompare(String(a.last_seen_at || a.checked_in_at))).slice(0, 100).map((fields) => ({
...
async function addObservation(airtable, schema, payload) {
  ...
  const record = await createAirtableRecord(airtable, schema, airtable.observationsTable, {
    ...
    class_no: classNo ? Number(classNo) : void 0,
    entry_no: entryNo ? Number(entryNo) : void 0,
    entry_order: entryOrder ? Number(entryOrder) : void 0,
...
async function logHideClasses(airtable, schema, payload) {
  const showNo = clean(payload.show_no || payload.showNo);
  const focusDay = isoDate(payload.focus_day || payload.focusDay);
  const classes = Array.isArray(payload.classes) ? payload.classes.map((item) => ({
...
async function findActiveRingCheckin(airtable, sessionId) {
  ...
  const records = await listAirtableRecords(airtable, airtable.ringCheckinsTable);
  return records.filter((record) => clean(record.fields?.session_id) === sessionId).filter((record) => clean(record.fields?.status).toLowerCase() === "active").sort((a, b) => String(b.fields?.last_seen_at || b.fields?.checked_in_at).localeCompare(String(a.fields?.last_seen_at || a.fields?.checked_in_at)))[0] || null;
}
