...
const appJs = `(async () => {
  ...
  const enrichmentUrl = config.enrichmentUrl || config.historyActionUrl || config.actionUrl || "/lp-history/enrichment";
  const profileUrl = config.profileUrl || config.profileContentUrl || "";
  debug("config", config);
