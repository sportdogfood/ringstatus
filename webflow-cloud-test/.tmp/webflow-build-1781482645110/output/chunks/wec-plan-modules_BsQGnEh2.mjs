...
async function planReport(airtable, requestUrl, planKey) {
  ...
  const [
    ...
    spec.planKey === "per_horse" ? listRecords(airtable, tables.pak_horses_roster, { view: selectedViewKey || void 0 }) : Promise.resolve([])
  ]);
  ...
  const planContext = {
    horseCount: spec.planKey === "per_horse" ? horseCountForView(horseRoster, selectedViewKey, selectedWave) : spec.planKey === "per_groom" ? waveHorseCount(selectedWave) : 0
  };
  const sourceRows = sourceRecords.map((record) => normalizeSource(record, spec)).filter((row) => row.active && sourceMatchesSelectedView(row, selectedViewKey, selectedWave));
  ...
  const itemRows = selectedViewKey === "not_going" ? [] : itemRecords.map((record) => normalizeItem(record, spec, sourceById, selectedWave, links, planContext)).filter((row) => row.active && itemMatchesActiveSource(row, activeSourceIds)).sort(compareItems);
  ...
  const laneControls = laneRecords.map((record) => normalizeLane(record, spec)).filter((lane) => lane.active).sort(compareLanes).map((lane) => ({ id: lane.id, key: lane.key, label: lane.label, active: lane.active, sortOrder: lane.sortOrder }));
  ...
  const comments = commentRecords.map(normalizeComment).filter((row) => row.active && commentMatchesPlan(row, spec, selectedWave, itemIds)).sort(compareChangeLikeRows);
...
async function planActionReport(airtable, requestUrl, planKey, payload) {
  const spec = planSpec(planKey);
...
async function sessionPingReport(airtable, requestUrl, payload) {
  const context = await loadContext(airtable);
  const tables = {
...
async function planPrintHtml(airtable, requestUrl, planKey) {
...
async function setItemCount(airtable, tables, spec, requestUrl, payload) {
  ...
  const packedValue = payload?.packed === void 0 ? null : wholeQuantityField(payload.packed);
  ...
  const session = await ensureSession(airtable, tables, payload);
  const nextPacked = exceptionState === "packed_max" ? current.need : clamp(packedValue === null ? current.packed + delta : packedValue, 0, current.need);
  ...
  const saved = current.link ? await patchAirtableRecord(airtable, tables.links.id, current.link.id, fieldsAllowedBySchema(linkFields, tableFieldNames(tables.links))) : await createAirtableRecord(airtable, tables.links.id, fieldsAllowedBySchema(linkFields, tableFieldNames(tables.links)));
...
async function adjustNeeded(airtable, tables, spec, requestUrl, payload) {
  ...
  const saved = current.link ? await patchAirtableRecord(airtable, tables.links.id, current.link.id, fieldsAllowedBySchema(fields, tableFieldNames(tables.links))) : await createAirtableRecord(airtable, tables.links.id, fieldsAllowedBySchema(fields, tableFieldNames(tables.links)));
  const log = await createPlanLog(airtable, tables, spec, {
...
async function itemActionContext(airtable, tables, spec, requestUrl, itemId) {
  ...
  const planContext = {
    horseCount: spec.planKey === "per_horse" ? horseCountForView(horseRosterRecords.map(normalizeRosterHorse), selectedViewKey, wave) : 0
  };
  const link = links.map((record) => normalizeLink(record, spec)).find(
    (row) => row.itemIds.includes(itemId) && (!wave?.id || !row.waveIds.length || row.waveIds.includes(wave.id))
  ) || null;
  const normalized = normalizeItem(item, spec, /* @__PURE__ */ new Map(), wave, link ? [link] : [], planContext);
  return {
    item,
    link,
    wave,
...
async function ensureSession(airtable, tables, payload) {
  ...
  return existing ? patchAirtableRecord(airtable, tables.pak_sessions.id, existing.id, allowed) : createAirtableRecord(airtable, tables.pak_sessions.id, allowed);
}
...
function tableViewOptionsForPlan(spec, selectedViewKey) {
  ...
  return { view: selectedViewKey || void 0 };
}
...
function normalizeItem(record, spec, sourceById, wave, links = [], planContext = {}) {
  ...
  const need = spec.planKey === "quantity" ? wholeQuantityField(link?.neededCurrent ?? computedNeed) : wholeQuantityField(computedNeed);
  ...
  return {
    ...
    wecListPlanIds: linkedIds(fields.wec_list_plans),
    wecPackWaveOneIds: linkedIds(fields.wec_pack_wave_one),
    wecPackWaveTwoIds: linkedIds(fields.wec_pack_wave_two),
    wecReturnWaveIds: linkedIds(fields.wec_return_wave),
    wecPackItemIds: linkedIds(fields.wec_pack_items),
...
function findItemLink(links, itemId, waveId) {
  return (links || []).find(
    (link) => link.active && link.itemIds.includes(itemId) && (!waveId || !link.waveIds.length || link.waveIds.includes(waveId))
  ) || null;
}
...
function fieldsAllowedBySchema(fields, fieldNames) {
  ...
  return Object.fromEntries(Object.entries(fields || {}).filter(([key, value]) => fieldNames.has(key) && value !== void 0));
}
...
export {
  ...
  planPrintHtml as b,
  corsHeaders as c,
  planActionReport as d,
  json as j,
  planReport as p,
  runtimeEnv as r,
  sessionPingReport as s
};
