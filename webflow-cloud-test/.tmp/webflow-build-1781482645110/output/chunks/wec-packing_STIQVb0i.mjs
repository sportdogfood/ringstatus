...
const OPTIONAL_LEGACY_FIELDS_BY_TABLE = {
  wec_pack_waves: /* @__PURE__ */ new Set(["groom_count_final", "horse_sanity"])
};
function runtimeEnv() {
  const localEnv = globalThis.process?.env || {};
  const astroEnv = Object.assign(__vite_import_meta_env__, { AIRTABLE_WEC_META_TABLE: "tbllJywsOstkqT5yZ", AIRTABLE_TOKEN: "patDeqY9NAQsuYx6q.7fd75026f0820373f62a72ca063f99b2203b9d873cb77aa3962637ab7bb0ec37", AIRTABLE_BASE_ID: "apptdhhNzduxm5gjn", OS: "Windows_NT" }) || {};
...
function airtableConfig(runtime = runtimeEnv()) {
...
async function loadWecContext(airtable) {
...
async function healthReport(airtable) {
...
async function stateReport(airtable, requestUrl) {
  ...
  const normalizedPackLists = packLists.filter((record) => !record.fields?.ignore).map(normalizePackList).filter(isPackingListLane).sort(comparePackLists);
  const normalizedHomeLists = homePackLists.filter((record) => !record.fields?.ignore).map(normalizePackList).sort(comparePackLists);
  const normalizedActiveLists = activePackLists.filter((record) => !record.fields?.ignore).map(normalizePackList).sort(comparePackLists);
  ...
  const normalizedHorses = horses.filter((record) => !selectedShowId || includesLinkedId(record.fields.wec_show, selectedShowId)).map(normalizeRosterHorse).sort(compareHorseRosterRows);
  const waveHorses = normalizedHorses.filter((horse) => isHorseInWave(horse, waveBase));
  new Set(waveHorses.map((horse) => horse.id));
  const normalizedWave = withEffectiveWaveCounts(waveBase, waveHorses);
  const filteredItems = selectedWave ? worksheetItems.filter((record) => isActiveWorksheetRow(record) && includesLinkedId(record.fields.pack_wave, selectedWave.id) && (!selectedShowId || includesLinkedId(record.fields.show, selectedShowId))) : [];
  ...
  const filteredHorses = worksheetHorses.filter((record) => itemIds.has(firstLinkedId(record.fields.packing_item)) || itemIds.has(firstLinkedId(record.fields.source_pack_item)) || selectedWave && includesLinkedId(record.fields.pack_wave, selectedWave.id));
  ...
  const items = sourceWorksheetRecords.map((record) => decoratePackingItem(
...
function buildHomeModules({ homeLists, purchaseOnsiteItems, items, sourcePackItemLookup, listPlanLookup, placeLookup, packListLookup, packingEvents, selectedShowId, selectedWaveId }) {
  ...
  const staticRows = purchaseOnsiteItems.map((record) => normalizePurchaseOnsiteTask(record, listPlanLookup, placeLookup, packListLookup, purchaseList.id, taskStates));
  const decisionRows = (items || []).filter(isPurchaseOnsiteItem).map((item) => normalizePurchaseOnsiteDecisionTask(item, sourcePackItemLookup, packListLookup, purchaseList.id, taskStates));
...
async function buildActiveListDetails(airtable, tables, activeLists, { homeModules, normalizedPackLists, sourcePackItemLookup, listPlanLookup, placeTagLookup, items }) {
  ...
  const details = await Promise.all((activeLists || []).map(async (list) => {
    ...
    if (module?.tasks) {
      ...
      const records = view ? await listOptionalViewRecords(airtable, tables.wec_places.id, view) : await listOptionalRecords(airtable, tables.wec_places);
...
function compareDetailRows(a, b) {
  return displayLabel(a.label || "").localeCompare(displayLabel(b.label || ""), void 0, { sensitivity: "base" }) || String(a.id || "").localeCompare(String(b.id || ""), void 0, { sensitivity: "base" });
}
...
function commentsFromEvents(events, { selectedWaveId, itemIds } = {}) {
  const selectedItems = itemIds instanceof Set ? itemIds : /* @__PURE__ */ new Set();
  return (events || []).map(normalizePackingComment).filter(Boolean).filter((comment) => commentBelongsToWave(comment, selectedWaveId, selectedItems));
}
function commentsFromCommentingTable(records, { selectedWaveId, itemIds } = {}) {
  const selectedItems = itemIds instanceof Set ? itemIds : /* @__PURE__ */ new Set();
  return (records || []).map(normalizeCommentingRecord).filter(Boolean).filter((comment) => commentBelongsToWave(comment, selectedWaveId, selectedItems));
}
...
function normalizeCommentingRecord(record) {
  ...
  const scopeType = slugify(fields.scope_type || fields.comment_scope || parsed.scopeType) || (linkedItemId ? "item" : linkedItemHorseId ? "item_horse" : linkedHorseId ? "horse" : linkedWaveId ? "wave" : "");
  const scopeId = clean(fields.scope_id || fields.comment_scope_id || parsed.scopeId) || linkedItemId || linkedItemHorseId || linkedHorseId || linkedWaveId || "";
...
function normalizePackingComment(record) {
  ...
  if (!scopeId || !comment) return null;
  return {
    id: record.id,
...
function normalizePurchaseOnsiteDecisionTask(item, sourcePackItemLookup, packListLookup, purchaseListId, taskStates) {
  const source = item.sourcePackItemIds.map((id) => sourcePackItemLookup.get(id)).filter(Boolean)[0];
...
function buildOnsiteListSummaries(tasks, packListLookup, purchaseList) {
  ...
  return [...listIds].map((id) => {
    ...
    const list = packListLookup.get(id) || (id === purchaseList.id ? purchaseList : { label: id });
    return {
      id,
...
function onsiteTaskStatesFromEvents(records, context = {}) {
  ...
  const relevant = [...records || []].filter((record) => ["onsite_task_done", "onsite_task_reopen"].includes(stringField(record.fields?.event_type))).filter((record) => eventMatchesContext(record, context)).sort((a, b) => compareText(a.createdTime, b.createdTime) || compareText(stringField(a.fields?.event), stringField(b.fields?.event)));
...
function horseKitStatesFromEvents(records, context = {}) {
  const states = /* @__PURE__ */ new Map();
  const relevant = [...records || []].filter((record) => ["horse_kit_packed", "horse_kit_not_needed", "horse_kit_reopened"].includes(stringField(record.fields?.event_type))).filter((record) => eventMatchesContext(record, context)).sort((a, b) => compareText(a.createdTime, b.createdTime) || compareText(stringField(a.fields?.event), stringField(b.fields?.event)));
  for (const record of relevant) {
    ...
    const horsePackState = eventType === "horse_kit_packed" ? "packed" : eventType === "horse_kit_not_needed" ? "not_needed" : "not_packed";
...
function printReportHtml(report, requestUrl) {
  const url = new URL(requestUrl);
  const target = clean(url.searchParams.get("target") || "overview");
  const horseId = clean(url.searchParams.get("horseId"));
  const title = horseId ? `${displayLabel(printHorseName(report.horses.find((horse) => horse.id === horseId)))} Packing List` : target === "overview" ? "WEC Packing Report" : `${printTargetTitle(report, target)} Packing List`;
  const body = horseId ? printHorsePackingPageHtml(report, horseId) : printBodyHtml(report, target);
...
function printPageContents(report, title, percent, pages) {
  return pages.map((content, index) => `
    <section class="packing-print-page">
      ${printGlobalHeaderHtml(report, title)}
      ${content}
...
function activePrintHorses(report) {
  ...
  const horseIds = /* @__PURE__ */ new Set();
  const horseKeys = /* @__PURE__ */ new Set();
  for (const member of members) {
    for (const horseId of member.horseIds || []) horseIds.add(horseId);
    if (member.barnName) horseKeys.add(slugify(member.barnName));
  }
  return (report.horses || []).filter((horse) => horseIds.has(horse.id) || horseKeys.has(slugify(printHorseName(horse)))).sort((a, b) => compareNumber(a.sortOrder, b.sortOrder) || printHorseName(a).localeCompare(printHorseName(b)));
}
...
function horseItemRows(report, horse) {
  return (report.items || []).flatMap((item) => {
    ...
    return members.filter((member) => horseMemberBelongsToHorse(member, horse)).map((member) => ({ item, member }));
  });
}
...
function printGlobalHeaderHtml(report, title, percent) {
  return `
    <header class="packing-print-head">
      ...
      <p>${escapeHtml(printStatusLine(report))}</p>
    </header>
  `;
}
...
function printWaveKey(report) {
  return clean(report.wave?.wave || "wave_one").toLowerCase().replace(/[_\s]+/g, "-");
}
function printDateDisplay() {
  return (/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { timeZone: "America/New_York" });
}
...
async function reconcileReport(airtable, requestUrl) {
  const url = new URL(requestUrl);
  const showId = clean(url.searchParams.get("showId"));
  const packWaveId = clean(url.searchParams.get("packWaveId"));
  const packWaveKey = clean(url.searchParams.get("packWaveKey") || url.searchParams.get("packWave") || url.searchParams.get("wave"));
  ...
  const filteredHorseMembers = worksheetHorses.filter((record) => itemIds.has(firstLinkedId(record.fields.packing_item)) || selectedWave && includesLinkedId(record.fields.pack_wave, selectedWave.id));
  ...
  const items = filteredItems.map((record) => decoratePackingItem(
    normalizePackingItem(record, horsesByItem.get(record.id) || [], listPlanLookup),
    packListLookup,
    sourcePackItemLookup,
    wave,
  ...
  for (const record of filteredHorseMembers) {
    ...
    const sourcePackItemIds = member.sourcePackItemIds.length ? member.sourcePackItemIds : linkedIds(parentItem?.fields?.source_pack_item);
    ...
    if (sourcePackItemId && (!expectedSourceHorseIds(sourceItem, waveHorseIds).has(member.horseIds[0]) || !memberHorse?.manualLock)) {
      staleHorseMembers.push({
        ...row,
        reason: memberHorse && !memberHorse.manualLock ? "horse_member_exists_without_horse_lock" : "horse_no_longer_expected_for_wave_or_source_item"
      });
  ...
  const quantityMismatches = items.filter((item) => item.quantityCalculation && !item.quantityCalculation.matchesFrozen).map((item) => ({
    id: item.id,
    itemName: item.name,
    itemId: item.itemId,
...
async function actionReport(airtable, requestUrl, payload) {
...
function shouldSkipReturnedState(action) {
...
async function applyCommentEvent(airtable, context, payload) {
  ...
  const itemRecord = itemId ? await findOptionalRecordInConfiguredView(airtable, tables.wec_packing_items, itemId) : null;
  ...
  if (tables.wec_commenting?.id) {
    const commentRecord = await createWecComment(airtable, context, {
      ...
      comment,
      itemRecord,
      showIds: showId ? [showId] : void 0,
      packWaveIds: packWaveId ? [packWaveId] : void 0
    });
...
function requiredMissingFields(tableName, allowedFields, schemaFields) {
  const optionalLegacy = OPTIONAL_LEGACY_FIELDS_BY_TABLE[tableName] || /* @__PURE__ */ new Set();
...
async function fetchAirtableListPage(airtable, table, url) {
  ...
  for (let attempt = 0; attempt <= retryDelays.length; attempt += 1) {
    ...
    const delay = Number.isFinite(retryAfter) && retryAfter > 0 ? retryAfter * 1e3 : retryDelays[attempt] + Math.floor(Math.random() * 250);
    await sleep(delay);
  }
  throw lastError;
}
...
async function resolveSourceActionItem(airtable, context, payload) {
  ...
  const currentPacked = ledgerState.hasEntries ? ledgerState.packed : wholeQuantityField(payload?.currentPacked ?? legacyPackingRecord?.fields?.quantity_packed);
  ...
  const itemName = clean(payload?.itemName) || sourceItemDisplayName(sourceItem) || stringField(legacyPackingRecord?.fields?.item_name) || sourceRecord.id;
  const payloadPackListIds = Array.isArray(payload?.packListIds) ? payload.packListIds.map(clean).filter(Boolean) : [];
  const packListIds = payloadPackListIds.length ? payloadPackListIds : linkedIds(legacyPackingRecord?.fields?.pack_list).length ? linkedIds(legacyPackingRecord.fields.pack_list) : sourceItem.packListIds;
  const fields = {
    item_name: itemName,
...
async function applyHorseSpecificItemPackState(airtable, context, actionItem, nextPackState, payload) {
  ...
  const waveHorses = horseRecords.filter((record) => !selectedShowId || includesLinkedId(record.fields?.wec_show, selectedShowId)).map(normalizeRosterHorse).filter((horse) => isHorseInWave(horse, wave)).sort(compareHorseRosterRows);
...
async function applyItemFieldUpdate(airtable, context, payload) {
  ...
  if (Object.prototype.hasOwnProperty.call(updateFields, "quantity_packed") || Object.prototype.hasOwnProperty.call(updateFields, "quantity_needed")) {
    ...
    const needed = Object.prototype.hasOwnProperty.call(updateFields, "quantity_needed") ? worksheetNeeded(fields) : actionNeeded(fields, payload);
    afterPacked = packed;
  ...
  const updated = await createPackingLedgerEntry(airtable, context, actionItem, {
    ...
    quantityDelta,
    quantityAfter: afterPacked,
    quantityNeeded: Object.prototype.hasOwnProperty.call(updateFields, "quantity_needed") ? updateFields.quantity_needed : void 0,
...
async function applyHorsePackState(airtable, tables, payload) {
  ...
  const rolledMembers = allMembers.filter((record) => includesLinkedId(record.fields?.packing_item, packingItemId)).map((record) => record.id === memberId ? { ...record, fields: { ...record.fields, quantity_packed: after, horse_pack_state: nextState } } : record);
...
async function applyHorseKitState(airtable, context, payload) {
  ...
  const itemRecord = await findOptionalRecordInConfiguredView(airtable, tables.wec_packing_items, packingItemId);
  const sourceRecord = tables.wec_pack_items?.id ? (await findRecordInConfiguredView(airtable, tables.wec_pack_items, sourcePackItemId)).record : null;
  const parentPackingItemId = itemRecord?.id || "";
  ...
  const existingMember = findHorseKitMemberRecord(allMembers, {
    itemHorseKey,
    packingItemId: parentPackingItemId,
    horseId,
    sourcePackItemId,
    packWaveId
  });
  ...
  if (parentPackingItemId) {
    const rolledMembers = allMembers.filter((record) => record.id !== memberRecord.id && includesLinkedId(record.fields?.packing_item, parentPackingItemId)).concat(memberRecord);
    const packedTotal = rolledMembers.reduce((sum, record) => sum + wholeQuantityField(record.fields?.quantity_packed), 0);
    const neededTotal = rolledMembers.reduce((sum, record) => sum + effectiveHorseMemberNeeded(record.fields), 0);
    const parentPackState = neededTotal > 0 && packedTotal >= neededTotal ? "packed" : "not_packed";
    updatedParent = await patchAirtableRecord(airtable, tables.wec_packing_items.id, parentPackingItemId, {
      quantity_packed: packedTotal,
      pack_state: parentPackState
    });
  }
  const event = await createPackingEvent(airtable, tables, {
    ...
    notes: [
      `source_pack_item: ${sourcePackItemId}`,
      `horse_id: ${horseId}`,
      parentPackingItemId ? `packing_item: ${parentPackingItemId}` : "",
...
function findHorseKitMemberRecord(records = [], criteria = {}) {
  ...
  return records.find((record) => {
    ...
    const sameHorseSourceWave = includesLinkedId(fields.horse, criteria.horseId) && includesLinkedId(fields.source_pack_item, criteria.sourcePackItemId) && (!criteria.packWaveId || includesLinkedId(fields.pack_wave, criteria.packWaveId));
    if (!sameHorseSourceWave) return false;
    return !criteria.packingItemId || includesLinkedId(fields.packing_item, criteria.packingItemId);
  }) || null;
}
async function upsertHorseKitMemberRecord(airtable, context, payload) {
  ...
  const fields = fieldsAllowedBySchema(compactFields({
    ...
    sort_order: sourceSortOrder || void 0
  }), fieldNames);
  if (!Object.keys(fields).length) throw new Error("wec_packing_item_horses_writable_fields_missing");
  const record = payload.existingMember ? await patchAirtableRecord(airtable, tables.wec_packing_item_horses.id, payload.existingMember.id, fields) : await createAirtableRecord(airtable, tables.wec_packing_item_horses.id, fields);
  return {
    id: record.id,
    fields: {
...
function withEffectiveWaveCounts(wave, waveHorses) {
  ...
  const currentHorseCount = usesWaveOneCount ? wave.countHorsesWaveOne : linkedHorseCount;
...
function normalizePlace(record, placeTagLookup = /* @__PURE__ */ new Map()) {
  const fields = record.fields || {};
...
const PLACE_ATTRIBUTE_EXCLUDED_FIELDS = /* @__PURE__ */ new Set([
  "place",
  "name",
...
function placeAttributeRows(fields) {
  ...
  }).filter(Boolean).sort((a, b) => a.label.localeCompare(b.label, void 0, { sensitivity: "base" }));
}
...
function buildPlaceLookup(records, placeTagLookup = /* @__PURE__ */ new Map()) {
...
function decorateSourcePackItem(item, placeLookup = /* @__PURE__ */ new Map()) {
...
function buildSourceWorksheetRecords({ sourceItems, ledgerState, selectedWave, selectedShowId, packListIds }) {
  ...
  return (sourceItems || []).filter((sourceItem) => isActiveSourceWorksheetItem(sourceItem, packListIds)).map((sourceItem) => sourceItemToWorksheetRecord(sourceItem, ledgerState.get(sourceItem.id), selectedWave, selectedShowId));
}
function isActiveSourceWorksheetItem(sourceItem, packListIds = /* @__PURE__ */ new Set()) {
...
function sourceItemToWorksheetRecord(sourceItem, state, selectedWave, selectedShowId) {
  ...
  return {
    ...
    fields: compactFields({
      ...
      quantity_needed: state?.quantityNeededFrozen ?? void 0,
...
function isTaskOnlySourceItem(sourceItem) {
  ...
  return plan === "purchase_onsite" || plan === "unresolved" || plan === "needs_attention" || !!sourceItem?.sourceFlags?.purchaseOnsite || !!sourceItem?.sourceFlags?.unresolved || !!sourceItem?.sourceFlags?.needsAttention;
}
...
function normalizeSourcePackItem(record, listPlanLookup = /* @__PURE__ */ new Map()) {
  const fields = record.fields || {};
  ...
  return {
    ...
    active: (fields.active === void 0 ? true : !!fields.active) && !fields.inactive && !fields.remove,
...
function normalizePackingItem(record, horseRecords, listPlanLookup = /* @__PURE__ */ new Map()) {
  const fields = record.fields || {};
  const listPlan = resolveListPlan(fields, listPlanLookup);
...
function decoratePackingItem(item, packListLookup, sourcePackItemLookup, wave, waveHorses = [], horseKitStateLookup = /* @__PURE__ */ new Map()) {
  const sourceItems = item.sourcePackItemIds.map((id) => sourcePackItemLookup.get(id)).filter(Boolean);
  const sourceItem = sourceItems[0];
  const horseSpecific = isHorseSpecificPlan(item, sourceItem);
  const horseMembers = horseSpecific ? expectedHorseKitMembers(item, sourceItem, wave, waveHorses, horseKitStateLookup) : item.horseMembers;
  ...
  const packed = horseSpecific ? horseMembers.filter(isPackedHorseMember).length : item.packed;
  const effectiveNeeded = horseSpecific ? horseMembers.reduce((sum, member) => sum + effectiveHorseMemberNeeded(member), 0) : wave?.countsLocked ? item.quantityNeededFrozen ?? item.needed : calculatedNeededForUnlockedItem(item, quantityCalculation);
  const effectiveItem = {
    ...item,
    horseMembers,
  ...
  return {
    ...effectiveItem,
    packListLabels: effectiveItem.packListIds.map((id) => packListLookup.get(id)?.label || "").filter(Boolean),
    sourceItems,
    quantityCalculation: {
      ...quantityCalculation,
      appliedNeeded: effectiveItem.needed,
      matchesApplied: Math.abs(numberField(quantityCalculation.calculatedNeeded) - numberField(effectiveItem.needed)) < 1e-4
    }
  };
}
...
function expectedHorseKitMembers(item, sourceItem, wave, waveHorses = [], horseKitStateLookup = /* @__PURE__ */ new Map()) {
  ...
  const existingByHorseId = /* @__PURE__ */ new Map();
  ...
  return expectedHorses.map((horse) => {
    ...
    const eventState = sourcePackItemId && horse.id ? horseKitStateLookup.get(`${sourcePackItemId}:${horse.id}`) : null;
    if (existing) {
      const horsePackState2 = eventState?.horsePackState || stringField(existing.horsePackState) || "not_packed";
      return {
        ...existing,
        needed: 1,
        packed: horsePackState2 === "packed" ? 1 : 0,
        horsePackState: horsePackState2,
...
function buildListSummaries(items, packLists) {
  ...
  return [...summaries.values()].filter((summary) => summary.rows > 0 || summary.itemCount > 0).map((summary) => ({
    ...summary,
...
function buildQuantityCalculation(item, sourceItem, wave, waveHorses = []) {
  ...
  if (plan === "per_horse") {
    ...
    return calculationRow({
      ...
      multiplierField: wave?.countsLocked ? "wec_pack_waves.horse_count" : wave?.countSource === "count_horses_wave_one" ? "wec_pack_waves.count_horses_wave_one" : "current wave horse count",
      base: perHorse,
...
function calculationRow({ plan, formula, sourceField, multiplierField, base, multiplier, calculatedNeeded, frozenNeeded, unit, countSource = "", countsLocked = false }) {
  return {
    ...
    countSource,
    countsLocked,
    matchesFrozen: Math.abs(numberField(calculatedNeeded) - numberField(frozenNeeded)) < 1e-4
  };
}
...
function expectedSourceHorseIds(sourceItem, waveHorseIds) {
  if (!sourceItem) return /* @__PURE__ */ new Set();
...
function isSafeToRemoveHorseMember(member, eventCount, horse) {
  return numberField(member.packed) === 0 && eventCount === 0 && member.horsePackState !== "packed" && !horse?.manualLock;
}
...
function waveKeyAliases(value) {
  ...
  if (!key) return [];
  const aliases = /* @__PURE__ */ new Set([key]);
  const wordToNumber = {
    one: "1",
    two: "2",
    three: "3",
...
function findSchemaTable(schema, name, idOrName) {
  ...
  return (schema.tables || []).find((table) => table.name === name || table.id === target || table.name === target);
}
...
function wholeQuantityField(value) {
  ...
  return Math.abs(number - Math.round(number)) < 1e-6 ? Math.round(number) : Math.ceil(number - 1e-6);
}
...
function compareText(a, b) {
  return clean(a).localeCompare(clean(b), void 0, { sensitivity: "base" });
}
...
export {
  ...
  healthReport as h,
  json as j,
  printReportHtml as p,
  reconcileReport as r,
  stateReport as s
};
