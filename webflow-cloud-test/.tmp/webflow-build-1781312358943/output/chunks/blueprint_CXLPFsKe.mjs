...
import { a as airtableConfig, j as json, c as corsHeaders } from "./wec-plan-modules_hXO0hoAk.mjs";
...
const WIRE_ROLES = /* @__PURE__ */ new Set([
  "entity_1",
  "entity_2",
  "links",
  "logs",
...
async function blueprintReport(airtable) {
...
function buildTests(tables, tableByName) {
  ...
  const headerWave = waveRows.find((record) => clean(record.fields?.wave) === "wave_one" || clean(record.fields?.pack_wave_key) === "wave_one") || waveRows.find((record) => record.fields?.active) || waveRows[0];
  const horseRows = tables.pak_horses_roster?.records || [];
  const testFieldRows = (tables.pak_fields?.records || []).map((record) => ({ id: record.id, fields: record.fields || {} })).filter((record) => record.fields.field_source_table === "pak_horses_roster" && /^test_button_/.test(record.fields.field_key || record.fields.data_rs_value || ""));
...
function buildGroupModel(tables) {
  ...
  for (const record of tables.pak_groups?.records || []) {
    ...
    for (const planId of planIds) {
      const plan = planById.get(planId) || { key: planId, label: planId };
...
function buildBlueprintValidationTests(tables, tableByName) {
  ...
  const groupModel = buildGroupModel(tables);
  const activeHorseFields = fieldRows.filter(
    (record) => record.fields?.field_source_table === "pak_horses_roster" && record.fields?.active
  );
  return [
    {
    ...
    },
    {
      ...
      detail: missingGroupComponentLinks.length ? missingGroupComponentLinks.map((record) => `${record.id}:${clean(record.fields?.component_key)}`).join(", ") : `${groupComponentRows.length} linked component rows`
    }
  ];
}
function buildNavModel(tables) {
  const pageRows = tables.pak_page_index?.records || [];
  const navRows = pageRows.map((record) => ({ id: record.id, fields: record.fields || {} })).filter((record) => record.fields.active && clean(record.fields.scope) === "navigation");
  const top = navRows.filter((record) => clean(record.fields.nav_scope) === "top").sort(sortByOrder).map(normalizeNavRow);
  const trays = {};
...
function navTargetUrl(key) {
  const targets = {
    ...
    horse_roster: "./wec-blueprint-preview.html#horse_roster",
    horse_profiles: "./wec-blueprint-preview.html#horse_profiles",
    horse_attributes: "./wec-blueprint-preview.html#horse_attributes",
...
function buildWireModel(tables) {
  ...
  const roles = wireRows.filter((record) => WIRE_ROLES.has(clean(record.fields.wire_role))).sort((a, b) => clean(a.fields.wire_role).localeCompare(clean(b.fields.wire_role))).map((record) => ({
    id: record.id,
  ...
  const draftAssignments = wireRows.filter((record) => {
    const role = clean(record.fields.wire_role);
    return role && !WIRE_ROLES.has(role);
  }).sort((a, b) => clean(a.fields.scope).localeCompare(clean(b.fields.scope)) || clean(a.fields.wire_role).localeCompare(clean(b.fields.wire_role))).map((record) => ({
    id: record.id,
    wireKey: clean(record.fields.wire_role),
    scope: clean(record.fields.scope),
    reason: "in pak_wire_index but not assigned in pak_wire_assignments"
  }));
  const assignments = assignmentRows.sort((a, b) => clean(a.fields.wire_key).localeCompare(clean(b.fields.wire_key)) || clean(a.fields.wire_role).localeCompare(clean(b.fields.wire_role))).map((record) => ({
    id: record.id,
...
function buildWireCoverage(suggestedAssignments) {
  ...
  for (const row of suggestedAssignments) {
    ...
    if (!byPlan.has(key)) byPlan.set(key, /* @__PURE__ */ new Set());
    byPlan.get(key).add(row.wireRole);
  }
  return [...byPlan.entries()].map(([plan, roles]) => ({
    plan,
    present: [...roles].sort(),
...
function buildSuggestedWireAssignments(tables) {
  ...
  return rows.sort(
    (a, b) => a.planLabel.localeCompare(b.planLabel) || a.wireRole.localeCompare(b.wireRole) || a.renderKey.localeCompare(b.renderKey)
  );
}
...
function buildHeaderChain(tables) {
  ...
  const component = componentRows.find(
    (record) => clean(record.fields?.component_key || record.fields?.component) === componentKey
  );
  const sourceKey = clean(stack?.fields?.source_record_key);
  const source = waveRows.find(
    (record) => clean(record.fields?.wave || record.fields?.wave_key || record.fields?.pack_wave_key) === sourceKey
...
function renderSlots(html, values) {
  if (!html) return "";
  return html.replace(/<span data-rs-slot="title"><\/span>/g, `<span data-rs-slot="title">${escapeHtml(values.title)}</span>`).replace(/<span data-rs-slot="subtitle"><\/span>/g, `<span data-rs-slot="subtitle">${escapeHtml(values.subtitle)}</span>`);
}
...
function sortByOrder(a, b) {
  return Number(a.fields?.sort_order || 0) - Number(b.fields?.sort_order || 0) || clean(a.fields?.page_key).localeCompare(clean(b.fields?.page_key));
}
