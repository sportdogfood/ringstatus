...
const WAVELESS_HORSE_KIT_VIEWS = /* @__PURE__ */ new Set(["all", "not_going"]);
...
async function horseKitReport(airtable, requestUrl) {
  ...
  if (!baseTables.pak_groups?.id) throw new Error("horse_kits_blueprint_missing:pak_groups");
  const groupRecords = await listHorseKitRecords(airtable, baseTables.pak_groups, {
    view: pakGroupsView,
    fields: horseKitReadFields(baseTables.pak_groups)
  });
  const groupStack = normalizePakGroupStack(groupRecords, { groupPrefix: "gp" });
  assertHorseKitBlueprint(groupStack);
  ...
  const [
    ...
    listHorseKitRecords(airtable, tables.wec_commenting, { extraFields: [rosterLinkFields.commentHorseFieldId] }),
    listOptionalHorseKitRecords(airtable, tables.comment_shorts),
    listHorseKitRecords(airtable, tables.pak_tabs),
    listHorseKitRecords(airtable, tables.wec_lanes, {
      view: "horse_specific",
      fields: horseKitReadFields(tables.wec_lanes)
    }),
    listHorseKitRecords(airtable, tables.pak_views)
  ]);
  ...
  if (!selectedWave && !WAVELESS_HORSE_KIT_VIEWS.has(packWaveKey)) throw new Error(`pack_wave_not_found:${packWaveKey || packWaveId}`);
  const selectedWavePakTabIds = new Set(selectedWave?.pakTabIds || []);
  const primaryTabs = tabRecords.map(normalizePakTab).filter((tab) => tab.active && (!selectedWavePakTabIds.size || selectedWavePakTabIds.has(tab.id))).sort(comparePakTabs).map((tab) => ({ id: tab.id, key: tab.key, label: tab.label, active: tab.active, sortOrder: tab.priority }));
  const laneControls = laneRecords.map(normalizeWecLane).filter((lane) => lane.active).sort(compareWecLanes).map((lane) => ({ id: lane.id, key: lane.key, label: lane.label, active: lane.active, sortOrder: lane.priority }));
  const secondaryControlPakViewIds = new Set(groupStack.activeRows.find((row) => row.renderKey === "secondary_controls")?.pakViewIds || []);
  const secondaryControls = viewRecords.map(normalizePakView).filter((view) => secondaryControlPakViewIds.has(view.id)).sort(comparePakViews).map((view) => ({ id: view.id, key: view.key, label: view.label, active: true, sortOrder: view.sortOrder }));
  ...
  const kits = kitRecords.map(normalizeKit).sort(compareKitTemplates).map((kit) => {
    ...
    const items = [...new Map([...linkedItems, ...reverseLinkedItems].map((item) => [item.id, item])).values()].filter((item) => item.status !== "inactive" && item.active !== false).sort(compareKitItems);
    return {
      id: kit.id,
      label: kit.label,
      name: kit.name,
  ...
  new Map(waveRecords.map((record) => {
    const wave = normalizeWave(record);
    return [wave.id, wave];
  }));
  const packingRows = packingKitRecords.map((record) => normalizePackingKit(record, {
    horseById,
    kitById,
    kitItemById,
    horseLinkField: rosterLinkFields.packingKitHorse
  })).filter((row) => row.horseIds.length && row.kitItemIds.length).filter((row) => !selectedWave?.id || row.packWaveIds.includes(selectedWave.id)).filter((row) => packingRowMatchesAssignedHorse(row, horseById)).map((row) => ({
    id: row.id,
    label: row.label,
  ...
  const comments = commentRecords.map((record) => normalizeComment(record, { horseLinkField: rosterLinkFields.commentHorse })).map((comment) => ({
    id: comment.id,
  ...
  return {
    ...
    source: {
      ...
      rosterRecordBaseUrl: tables.pak_horses_roster?.id ? `https://airtable.com/${encodeURIComponent(airtable.baseId)}/${encodeURIComponent(tables.pak_horses_roster.id)}/` : "",
...
async function horseKitActionReport(airtable, requestUrl, payload) {
...
function horseKitPrintHtml(report, requestUrl) {
  const url = new URL(requestUrl);
  const horseId = clean(url.searchParams.get("horseId"));
  const title = horseId ? `${horseKitPrintHorseName(findHorseForPrint(report, horseId))} Kit Items` : `${clean(report?.wave?.reportTitle) || "WEC PACK"} Horse Kits`;
  const body = horseId ? horseKitPrintHorseBody(report, horseId) : horseKitPrintListBody(report);
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
function horseKitTables(context, groupStack = null) {
  ...
  const groupTable = (renderKey) => requiredPakGroupPhysicalTableName(groupStack, renderKey);
  return {
    wec_lanes: physicalTableConfig(context, groupTable("lane_source")),
    wec_pack_waves: physicalTableConfig(context, "wec_pack_waves"),
    pak_kits: physicalTableConfig(context, groupTable("kit_source")),
    pak_kit_items: physicalTableConfig(context, groupTable("drawer_items")),
    pak_horses_roster: physicalTableConfig(context, groupTable("main_table")),
    horse_packing_kits: physicalTableConfig(context, groupTable("state_links")),
    horse_kit_changes: physicalTableConfig(context, groupTable("change_log")),
    wec_commenting: physicalTableConfig(context, groupTable("comments")),
    comment_shorts: physicalTableConfig(context, "comment_shorts", true),
    pak_tabs: physicalTableConfig(context, "pak_tabs", true),
    pak_views: physicalTableConfig(context, groupTable("secondary_controls")),
    pak_groups: physicalTableConfig(context, "pak_groups")
  };
}
function assertHorseKitTables(tables) {
  for (const key of ["wec_pack_waves", "wec_lanes", "pak_groups", "pak_tabs", "pak_views", "pak_horses_roster", "pak_kits", "pak_kit_items", "horse_packing_kits", "horse_kit_changes", "wec_commenting"]) {
...
function horseKitRosterLinkFields(tables) {
  ...
  if (!packingKitHorse.name) throw new Error("missing_horse_packing_kits_roster_link");
  if (!commentHorse.name) throw new Error("missing_wec_commenting_roster_link");
  return {
...
function linkedRecordField(tableConfig, targetTableConfig) {
  const targetTableId = targetTableConfig?.id || "";
  const schemaFields = tableConfig?.schemaFields || [];
  const linked = schemaFields.find(
    (field) => field.type === "multipleRecordLinks" && field.options?.linkedTableId === targetTableId
  );
...
function normalizePakGroupStack(records = [], options = {}) {
  ...
  const rows = records.map((record, index) => normalizePakGroupRow(record, index)).filter((row) => row.groupPrefix === groupPrefix).sort((a, b) => a.sortOrder - b.sortOrder || a.stack - b.stack || a.sourceIndex - b.sourceIndex);
  return {
    groupPrefix,
    rows,
...
function normalizePakGroupRow(record, index = 0) {
  ...
  const physicalTableName = clean(fields.physical_table);
  return {
    id: record.id,
    sourceIndex: index,
...
function assertHorseKitBlueprint(groupStack) {
  const activeKeys = new Set((groupStack.activeRows || []).map((row) => row.renderKey));
  for (const key of REQUIRED_HORSE_KIT_STACK_KEYS) {
    if (!activeKeys.has(key)) throw new Error(`horse_kits_blueprint_missing_stack:${key}`);
  }
  for (const key of ["main_table", "drawer_items", "state_links", "change_log", "kit_source", "comments", "lane_source", "secondary_controls"]) {
    requiredPakGroupPhysicalTableName(groupStack, key);
  }
}
function requiredPakGroupPhysicalTableName(groupStack, renderKey) {
  const normalizedKey = slugify(renderKey);
  const row = (groupStack?.rows || []).find((candidate) => candidate.renderKey === normalizedKey);
  if (!row) throw new Error(`horse_kits_blueprint_missing_stack:${normalizedKey}`);
  if (!row.physicalTableName) throw new Error(`horse_kits_blueprint_missing_physical_table:${normalizedKey}`);
...
async function applyPackingKitState(airtable, tables, payload) {
  ...
  const existing = rowId ? (await findRecordInConfiguredView(airtable, tables.horse_packing_kits, rowId)).record : existingRows.find(
    (candidate) => includesLinkedId(candidate.fields?.[horseLinkField], horseId) && includesLinkedId(candidate.fields?.pack_wave, packWaveId) && includesLinkedId(candidate.fields?.pak_kit_items, kitItemId) && (!kitId || includesLinkedId(candidate.fields?.pak_kits, kitId))
  );
  const horse = rowId ? null : horses.find((record2) => record2.id === horseId);
  const itemRecord = rowId ? null : pakKitItems.find((record2) => record2.id === kitItemId);
  if (!existing) {
  ...
  const updated = existing ? await patchAirtableRecord(airtable, tables.horse_packing_kits.id, record.id, updateFields) : await createAirtableRecord(airtable, tables.horse_packing_kits.id, compactFields({
...
async function saveHorseKitComment(airtable, tables, payload) {
  ...
  const commentShort = commentShortId && tables.comment_shorts?.id ? await findOptionalRecordInConfiguredView(airtable, tables.comment_shorts, commentShortId) : null;
  ...
  const before = commentId ? await findOptionalRecordInConfiguredView(airtable, tables.wec_commenting, commentId) : null;
  const fields = compactFields({
    event: before ? void 0 : `horse_kit_comment:${scopeType}:${scopeId}:${Date.now()}`,
    event_type: before ? void 0 : "horse_kit_comment",
    ...
    created_at: before ? void 0 : (/* @__PURE__ */ new Date()).toISOString().slice(0, 10),
    created_by: "webflow",
    notes: clean(payload?.notes)
  });
  ...
  const saved = commentId ? await patchAirtableRecord(airtable, tables.wec_commenting.id, commentId, allowedFields) : await createAirtableRecord(airtable, tables.wec_commenting.id, allowedFields);
  return { saved };
}
...
function normalizePackingKit(record, lookups = {}) {
  ...
  return {
    id: record.id,
    label: stringField(fields.horse_packing_kit) || [horse?.name, kitItem?.label].filter(Boolean).join(" - ") || record.id,
    horseIds,
...
function horseKitPrintHorseRows(report, horse, kitId) {
  ...
  const items = (report?.kitItems || []).filter((item) => itemIds.has(item.id)).filter((item) => !kitId || !item.kitIds?.length || item.kitIds.includes(kitId)).sort((a, b) => compareText(horseKitItemPrintLabel(a), horseKitItemPrintLabel(b)));
...
function horseKitPrintCounts(report, horse, kitId) {
  const itemIds = new Set(horse?.pakKitItemIds || []);
  const items = (report?.kitItems || []).filter((item) => itemIds.has(item.id)).filter((item) => !kitId || !item.kitIds?.length || item.kitIds.includes(kitId));
  let packed = 0;
  let notNeeded = 0;
  for (const item of items) {
...
function horseKitPrintItemState(report, horseId, itemId, kitId) {
  const row = (report?.packingRows || []).find(
    (candidate) => (candidate.horseIds || []).includes(horseId) && (candidate.kitItemIds || []).includes(itemId) && (!kitId || !candidate.kitIds?.length || candidate.kitIds.includes(kitId))
  );
...
function horseKitPrintAutoScript(url) {
  ...
  return ["1", "true", "yes"].includes(autoprint) ? `<script>window.addEventListener("load",function(){setTimeout(function(){window.print();},120);});<\/script>` : "";
}
...
function comparePakViews(a, b) {
  const order = /* @__PURE__ */ new Map([["all", 0], ["wave_one", 1], ["wave_two", 2], ["not_going", 3]]);
  return compareNumber(order.has(a.key) ? order.get(a.key) : 99, order.has(b.key) ? order.get(b.key) : 99) || compareText(a.label || a.key, b.label || b.key);
}
...
function compareHorsePackingRows(a, b) {
  return compareText(a.horseName, b.horseName) || compareText(a.kitLabel, b.kitLabel) || compareNumber(a.sortOrder, b.sortOrder) || compareText(a.itemLabel, b.itemLabel) || compareText(a.id, b.id);
}
...
export {
  airtableConfig as a,
  horseKitPrintHtml as b,
  corsHeaders as c,
  horseKitActionReport as d,
  horseKitReport as h,
  json as j
};
