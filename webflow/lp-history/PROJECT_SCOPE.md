...
- Edit mode: `http://127.0.0.1:8787/lp-history-webflow-hydrate-template.html?key=YOUR_EDIT_KEY`
...
Current workflow:
1. Open local edit URL with `?key=YOUR_EDIT_KEY`.
...
- Authentication beyond the simple `?key=YOUR_EDIT_KEY` edit-mode gate.
...
- Edit mode loads at the same URL with `?key=YOUR_EDIT_KEY`.
