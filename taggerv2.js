...
function resolveModeFromClock(clock) {
  ...
  // Two-shift logic:
  // Mon = HOLDOVER, HOLDOVER
  // Tue = HOLDOVER until 4:59 PM, NIGHT starting 5:00 PM
  // Wed–Sat = DAY until 4:59 PM, NIGHT starting 5:00 PM
  // Sun = DAY until 4:59 PM, HOLDOVER starting 5:00 PM

  if (dow === 1) return "HOLDOVER"; // Mon

  if (dow === 2) { // Tue
    return minuteOfDay >= (17 * 60) ? "NIGHT" : "HOLDOVER";
  }

  if (dow === 3 || dow === 4 || dow === 5 || dow === 6) { // Wed-Sat
    return minuteOfDay >= (17 * 60) ? "NIGHT" : "DAY";
  }

  if (dow === 0) { // Sun
    return minuteOfDay >= (17 * 60) ? "HOLDOVER" : "DAY";
  }
