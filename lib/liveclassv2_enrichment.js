...
function normalizeLiveClassDataPayload(payload) {
  ...
  return {
    ...
    rows: rows.map((row) => ({
      ...
      placing: numOrNull(isBlank(row?.ActualPos) ? row?.Pos : row?.ActualPos),
      time_one: numOrNull(row?.R1T),
      time_two: numOrNull(row?.JOT),
      time_three: numOrNull(row?.R3T),
      time_fault_one: numOrNull(row?.R1TF),
      faults_one: numOrNull(row?.R1F),
      time_faults_two: numOrNull(row?.JOTF),
      faults_two: numOrNull(row?.JOF),
      score1: numOrNull(row?.J1),
      score2: numOrNull(row?.J2),
      score3: numOrNull(row?.J3),
