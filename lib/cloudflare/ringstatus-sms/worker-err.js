...
const WORKER_VERSION = "ringstatus-sms-livescorewidget-ringlane-v3-offset-2026-05-09";
...
async function buildLiveScoreWidgetRingReply({ env, decision, addDbg }) {
  ...
  const asOf = formatUnixOffsetShort(maxUpdated, showOffsetMinutes) || nowShort(tz);
  const asOfMinutes = unixMinutesWithOffset(maxUpdated, showOffsetMinutes);

  addDbg(`showOffsetMinutes=${showOffsetMinutes}`);
  ...
  if (livenow.length) {
    ...
    const first = buildLiveNowLine(nowGroup, showOffsetMinutes, addDbg);
...
function buildLiveNowLine(group, showOffsetMinutes, addDbg) {
  ...
  const endsTxt = deriveLiveEndsText(group, showOffsetMinutes, addDbg);
...
function deriveLiveEndsText(group, showOffsetMinutes, addDbg) {
  const total = numberOrNull(group?.total);
  const gone = numberOrNull(group?.gone);
  const left = total == null || gone == null ? null : Math.max(0, total - gone);
  ...
  if (gone != null && gone > 4) {
    ...
    const updatedMinutes = unixMinutesWithOffset(group?.curr_updated_at, showOffsetMinutes);
...
function deriveStartsInText(group, asOfMinutes) {
  ...
  const diff = startMinutes - asOfMinutes;
