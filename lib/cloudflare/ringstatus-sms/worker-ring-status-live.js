// ringstatus-sms — worker-ring-status-live.js
// Forked module for SMS ring-status replies ("ring 4", "Hunter 1", etc.).
// Starting point: live endpoint implementation from the 2026-06-05 review.
// Keep isolated from worker.js / worker-live.js until the ring-status lane is verified.
//
...
// - SHOW_TIME_ZONE_OFFSET (default -240)
// - DEBUG_SMS="1"
// - UNKNOWN_REPLY
// - WATCH_TRIPS_URL
// - RIDER_LOOKUP_URL
...
const WORKER_VERSION = "ringstatus-sms-livescorewidget-ringlane-v4-tz-2026-05-10";
...
async function buildLiveScoreWidgetRingReply({ env, decision, addDbg }) {
  ...
  if (livenow.length) {
    ...
    const nextLine = nextGroup ? buildUpcomingLine("Next", nextGroup, asOfMinutes) : "";
    const followingLine = followingGroup ? buildUpcomingLine("Following", followingGroup, asOfMinutes) : "";
    const next = [nextLine, followingLine].filter(Boolean).join("\n\n");
...
function getShowOffsetMinutes(env) {
  const raw = String(env.SHOW_TIME_ZONE_OFFSET || env.TIME_ZONE_OFFSET || "-240").trim();
  const n = Number(raw);
  return Number.isFinite(n) ? n : -240;
}

function getShowTimeZone(env) {
  return (
    String(env.SHOW_TZ || "").trim() ||
    String(env.SHOW_TIME_ZONE || "").trim() ||
    "America/New_York"
  );
}

function formatUnixOffsetShort(unixSeconds, offsetMinutes) {
  const n = Number(unixSeconds);
  if (!Number.isFinite(n) || n <= 0) return "";

  const d = new Date((n + offsetMinutes * 60) * 1000);
...
function unixMinutesWithOffset(unixSeconds, offsetMinutes) {
  const n = Number(unixSeconds);
...
function healthResponse(request, env) {
  ...
  const showTimeZone = getShowTimeZone(env);
  const showOffsetMinutes = getShowOffsetMinutes(env);
  ...
  const body = {
    ...
    show_time_zone_offset: showOffsetMinutes,
