...
// - If no ring is recognized, try rider lookup against watch_trips.json
...
async function tryRiderLane({ inboundBody, twowayId, phone, env, addDbg, withDbg }) {
  ...
  if (!matches.length) {
    if (!looksLikeRiderQuery(inboundBody)) return null;
  ...
  const msg = buildRiderReply(picked);
...
function buildRiderReply(record) {
  const rider = cleanName(record?.riderName || "Rider");
  const ringNumber = String(record?.ring_number || "").trim();
  const ringText = ringNumber ? `Ring ${ringNumber}` : cleanName(record?.ringName || "Ring");
  const horse = cleanName(record?.horseName || "Horse");
  const className = cleanName(record?.class_name || record?.group_name || "Class");
  const classNumber = String(record?.class_number || "").trim();
  const backNumber = String(record?.backNumber || record?.entryNumber || "").trim();
  const scheduled = String(record?.latestStart || formatDisplayTime(record?.estimated_start_time) || "").trim();
  const latestGo = String(record?.latestGO || "").trim();
  const ringWalk = String(record?.ringWalk || "").trim();

  return (
    `${rider} — ${ringText}. ${horse}. ${className}` +
    (classNumber ? ` (${classNumber})` : "") +
    `. Back #${backNumber}. Scheduled ${scheduled}. Latest go ${latestGo}. Ring walk ${ringWalk} min.`
  );
}
...
const RINGS = [
  { ring_number: 1, sms_label: "Derby", ring_id: 10, aliases: ["derby", "derby field", "ring 1", "r1", "1"] },
  { ring_number: 2, sms_label: "Annex", ring_id: 3, aliases: ["annex", "ring 2", "r2", "2"] },
  { ring_number: 3, sms_label: "Covered", ring_id: 44, aliases: ["covered", "ring 3", "r3", "3"] },
  { ring_number: 4, sms_label: "Village", ring_id: 13, aliases: ["village", "ring 4", "r4", "4"] },
  { ring_number: 5, sms_label: "Intl", ring_id: 51, aliases: ["intl", "international", "wellington international", "ring 5", "r5", "5"] },
  { ring_number: 6, sms_label: "Grand", ring_id: 9, aliases: ["grand", "mische", "ring 6", "r6", "6"] },
  { ring_number: 7, sms_label: "Rost", ring_id: 37, aliases: ["rost", "ring 7", "r7", "7"] },
  { ring_number: 8, sms_label: "Mogavero", ring_id: 52, aliases: ["mogavero", "ring 8", "r8", "8"] },
  { ring_number: 9, sms_label: "Ring 6", ring_id: 25, aliases: ["ring 9", "r9", "9"] },
  { ring_number: 10, sms_label: "Ring 7", ring_id: 56, aliases: ["ring 10", "r10", "10"] },
  { ring_number: 11, sms_label: "Ring 8", ring_id: 57, aliases: ["ring 11", "r11", "11"] },
  { ring_number: 12, sms_label: "Ring 9", ring_id: 58, aliases: ["ring 12", "r12", "12"] },
  { ring_number: 13, sms_label: "Ring 10", ring_id: 53, aliases: ["ring 13", "r13", "13"] },
  { ring_number: 14, sms_label: "Ring 11", ring_id: 22, aliases: ["ring 14", "r14", "14"] },
  { ring_number: 15, sms_label: "30 Ring", ring_id: 2, aliases: ["30 ring", "ring 30", "pista 30", "ring 15", "r15", "15"] },
];
