...
const WORKER_VERSION = "ringstatus-sms-proxy-lookup-2026-04-19";
...
async function tryRiderLane({ inboundBody, twowayId, phone, env, addDbg, withDbg }) {
  ...
  for (const lane of lanes) {
    ...
    const lookupUrl = buildLookupUrl(baseUrl, intent.query);
...
async function tryLegacyRiderLane({ inboundBody, twowayId, phone, env, addDbg, withDbg }) {
  const needle = normalize(inboundBody);
  if (!needle) return null;

  const isRiderQuery = looksLikeRiderQuery(inboundBody);
  ...
  if (!matches.length) {
    ...
    const msg = riderNameValues.length
      ? `We could not find a current trip for "${cleanName(inboundBody)}". Try the rider name exactly as listed, or send a ring like "Ring 10" or "Grand".`
...
function parseLookupIntent(input) {
  ...
  if (!norm) return { shouldLookup: false, lane: "", query: "" };
  if (/\b(ring|today|tomorrow|yesterday|help|status|schedule)\b/.test(norm)) {
    return { shouldLookup: false, lane: "", query: "" };
  }
  ...
  if (!queryNorm || /^\d+$/.test(queryNorm)) return { shouldLookup: false, lane: "", query: "" };
  if (!/^[a-z0-9]+(?: [a-z0-9]+){0,5}$/.test(queryNorm)) return { shouldLookup: false, lane: "", query: "" };
  ...
  return { shouldLookup: true, lane, query };
}
...
function buildLookupUrl(baseUrl, query) {
  ...
  try {
    ...
    return `${input}${sep}q=${encodeURIComponent(query)}&_sms_ts=${Date.now()}`;
  }
}
...
function buildLookupReply(payload, lane, fallbackName) {
  ...
  const label = cleanName(firstText(record?.replyLabel, "Next"));
  ...
  const order = firstText(record?.orderOfGo, record?.runningOrderOfGo);
  const goneIn = firstText(record?.goneIn);
  ...
  const goParts = [];
  if (go) goParts.push(`Go: ${go}`);
  if (order) goParts.push(`Order: ${order}`);
  if (goneIn) goParts.push(`GoneIn: ${goneIn}`);
  if (minutesTxt) goParts.push(`Mins till go: ${minutesTxt}`);
...
function legacyReplyLabel(record) {
  const status = normalize(firstText(record?.latestStatus, record?.status));
  ...
  const goneFlag = goneIn && !["0", "false", "no", "n"].includes(goneIn);
