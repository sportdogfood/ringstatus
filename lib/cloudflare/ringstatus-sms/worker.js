...
const WORKER_VERSION = "ringstatus-sms-lookup-style-2026-04-19";
...
const RIDER_LOOKUP_UNAVAILABLE_REPLY =
  `We found the rider lookup feed, but it does not include rider names right now. Try a ring like "Ring 10" or "Grand", or try the rider again shortly.`;

const RIDER_LOOKUP_ERROR_REPLY =
  `Rider lookup is temporarily unavailable. Try the rider again shortly, or send a ring like "Ring 10" or "Grand".`;
...
async function tryRiderLane({ inboundBody, twowayId, phone, env, addDbg, withDbg }) {
  ...
  for (const lane of lanes) {
    ...
    if (!payload.selected || matches <= 0) continue;
  ...
  const msg = `We could not find a current trip for "${cleanName(intent.query)}". Try the rider or horse name exactly as listed, or send a ring like "Ring 10" or "Grand".`;
  return postAndReturnMessage({
    twowayId,
    inboundBody,
    phone,
...
async function tryLegacyRiderLane({ inboundBody, lookupBody, twowayId, phone, env, addDbg, withDbg }) {
  ...
  if (!matches.length) {
    ...
    const msg = riderNameValues.length
      ? `We could not find a current trip for "${cleanName(bodyForLookup)}". Try the rider name exactly as listed, or send a ring like "Ring 10" or "Grand".`
...
function buildLookupAllReply(payload, lane, fallbackName) {
  ...
  const title = `${subject} - all classes as of ${nowShort("America/New_York")}`;
  const lines = records.map((record, index) => {
    const { first, next } = buildLookupRecordParts(record, lane, true);
    return `${index + 1}. ${[first, next].filter(Boolean).join("\n")}`;
  });
  const matches = Number(payload?.matches ?? records.length);
  const more = matches > records.length ? `+${matches - records.length} more matches. Send "${subject} 441" for a specific class.` : "";
  const first = lines[0] || "No current classes found.";
  const next = [...lines.slice(1), more].filter(Boolean).join("\n\n");
  const fullText = [title, ...lines, more].filter(Boolean).join("\n\n");

  return { title, first, next, fullText };
}

function buildLookupRecordParts(record, lane, compact) {
  const label = lookupReplyLabel(record, compact ? "Class" : "Next");
  ...
  const goParts = buildGoParts(record);
  ...
  const next = [
    goParts.join(" | "),
    resultParts.length ? `Result: ${resultParts.join(" | ")}` : "",
  ].filter(Boolean).join("\n");
...
function buildClassStatsParts(record) {
  ...
  if (endsTxt) parts.push(`Ends: ${endsTxt}`);
...
function buildLookupContextParts(record, lane) {
  const contextParts = [];
  if (lane === "rider" && record?.horseName) contextParts.push(`Horse: ${cleanName(record.horseName)}`);
  if (lane === "horse" && record?.riderName) contextParts.push(`Rider: ${cleanName(record.riderName)}`);
  if (record?.ringName) contextParts.push(`Ring: ${cleanName(record.ringName)}`);
  return contextParts;
}

function buildGoParts(record) {
  const order = firstText(record?.orderOfGo, record?.rs_order_of_go, record?.runningOrderOfGo, record?.rs_running_order_of_go);
  const goneIn = firstText(record?.goneIn, record?.rs_gone_in);
  const go = formatDisplayTime(firstText(record?.goTimeDisplay, record?.goTime, record?.latestGO, record?.estimatedGO));
  const minutesToGo = numberOrNull(record?.minutesToGo, record?.rs_min_till_go);
  const minutesTxt = formatLookupMinutes(minutesToGo, record?.tripCategory);
  const parts = [];

  if (order && go) parts.push(`Go: ${go}`);
  if (order) parts.push(`Order: ${order}`);
  if (shouldDisplayFlag(goneIn)) parts.push(`GoneIn: ${goneIn}`);
  if (order && minutesTxt) parts.push(`Mins till go: ${minutesTxt}`);
...
function buildResultParts(record) {
  const resultParts = [];
  if (record?.lastPlacing || record?.lastPlace) resultParts.push(`Place: ${firstText(record?.lastPlacing, record?.lastPlace)}`);
  if (record?.lastScore) resultParts.push(`Score: ${record.lastScore}`);
  if (record?.lastTime) resultParts.push(`Time: ${record.lastTime}`);
  return resultParts;
}
...
function lookupReplyLabel(record, fallback) {
  ...
  const goneIn = normalize(firstText(record?.goneIn, record?.rs_gone_in));
  const hasResult = Boolean(record?.hasResult || firstText(record?.lastScore, record?.lastTime, record?.lastPlace, record?.lastPlacing));
  const goneFlag = shouldDisplayFlag(goneIn);

  if (hasResult || goneFlag || status === "completed") return "Completed";
  if (status === "underway" || status === "in progress") return "Now";
  return cleanName(firstText(record?.replyLabel, fallback));
}
...
function buildLegacyRiderReply(record) {
  ...
  const goParts = [];
  if (order && go) goParts.push(`Go: ${go}`);
  if (order) goParts.push(`Order: ${order}`);
  if (shouldDisplayFlag(goneIn)) goParts.push(`GoneIn: ${goneIn}`);
  if (order && minutesTxt) goParts.push(`Mins till go: ${minutesTxt}`);
  ...
  const contextParts = [];
  if (horse) contextParts.push(`Horse: ${horse}`);
  if (ringText) contextParts.push(`Ring: ${ringText}`);
...
function legacyReplyLabel(record) {
  const status = normalize(firstText(record?.latestStatus, record?.status, record?.rs_status));
  const goneIn = normalize(firstText(record?.goneIn, record?.rs_gone_in));
  const hasResult = Boolean(firstText(record?.lastScore, record?.lastTime, record?.lastPlace, record?.lastPlacing));
  const goneFlag = shouldDisplayFlag(goneIn);
...
const RINGS = [
  ...
  { ring_number: 5, sms_label: "Intl", ring_id: 51, aliases: ["intl", "international"] },
  { ring_number: 6, sms_label: "Grand", ring_id: 9, aliases: ["grand", "mische"] },
  { ring_number: 7, sms_label: "Rost", ring_id: 37, aliases: ["rost"] },
  { ring_number: 8, sms_label: "Mogavero", ring_id: 52, aliases: ["mogavero"] },
  { ring_number: 9, sms_label: "Ring 6", ring_id: 25, aliases: ["ring 6", "r6", "6"] },
  { ring_number: 10, sms_label: "Ring 7", ring_id: 56, aliases: ["ring 7", "r7", "7"] },
  { ring_number: 11, sms_label: "Ring 8", ring_id: 57, aliases: ["ring 8", "r8", "8"] },
  { ring_number: 12, sms_label: "Ring 9", ring_id: 58, aliases: ["ring 9", "r9", "9"] },
  { ring_number: 13, sms_label: "Ring 10", ring_id: 53, aliases: ["ring 10", "r10", "10"] },
  { ring_number: 14, sms_label: "Ring 11", ring_id: 22, aliases: ["ring 11", "r11", "11"] },
  { ring_number: 15, sms_label: "30 Ring", ring_id: 2, aliases: ["30 ring", "ring 30", "pista 30"] },
];
