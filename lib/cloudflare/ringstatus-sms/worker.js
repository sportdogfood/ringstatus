...
const WORKER_VERSION = "ringstatus-sms-rider-debug-2026-04-18";
...
async function tryRiderLane({ inboundBody, twowayId, phone, env, addDbg, withDbg }) {
  ...
  if (!resp.ok) {
    ...
    return postAndReturnMessage({
      ...
      inboundBody,
      phone,
      msg: buildRiderLookupFailureReply(`feed status ${resp.status}`, watchTripsUrl),
      withDbg,
    });
  }
  ...
  if (!payload) {
    ...
    return postAndReturnMessage({
      ...
      inboundBody,
      phone,
      msg: buildRiderLookupFailureReply("bad JSON", watchTripsUrl),
      withDbg,
    });
  }
...
function buildRiderReply(record) {
  ...
  const ringText = ringNumber ? `Ring ${ringNumber}` : cleanName(firstText(record?.ringName, "Ring"));
  ...
  if (ringWalk) parts.push(`Ring walk ${ringWalk} min`);
