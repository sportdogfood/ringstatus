...
const WORKER_VERSION = "ringstatus-sms-gonein-now-2026-04-19";
...
function buildClassStatsParts(record) {
  ...
  if (completed || isGoneInRecord(record)) return parts;
...
function buildLookupContextParts(record, lane) {
  ...
  if (!isClassCompletedRecord(record) && isGoneInRecord(record)) contextParts.push("Gone in: Y");
  return contextParts;
}
...
function buildGoParts(record) {
  ...
  if (isClassCompletedRecord(record) || goneFlag) return parts;
...
function buildResultParts(record) {
  ...
  if (score) resultParts.push(`Score: ${score}`);
...
function buildLegacyRiderReply(record) {
  ...
  const firstParts = [];
  if (!completed && !isGoneInRecord(record)) {
