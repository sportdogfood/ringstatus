...
const WORKER_VERSION = "ringstatus-sms-state-logic-2026-04-19";
...
function buildGoParts(record) {
  ...
  if (hasDisplayResult(record) || status === "completed") return parts;
  if (goneFlag) return ["Gone in: Y"];
...
function lookupReplyLabel(record, fallback) {
  ...
  const hasResult = hasDisplayResult(record);
  const goneFlag = shouldDisplayFlag(goneIn);
...
function hasDisplayResult(record) {
  return Boolean(record?.hasResult || firstText(record?.lastScore, record?.lastTime, record?.lastPlace, record?.lastPlacing));
}
...
function buildLegacyRiderReply(record) {
  ...
  const goParts = [];
  if (!hasDisplayResult(record) && status !== "completed") {
    if (goneFlag) {
      goParts.push("Gone in: Y");
    } else {
