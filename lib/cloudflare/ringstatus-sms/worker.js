...
const WORKER_VERSION = "ringstatus-sms-exact-rider-format-2026-04-19";
...
function buildGoParts(record) {
  ...
  if (isCompletedRecord(record)) return parts;
  if (goneFlag) return ["Gone in: Y"];

  if (order && go) parts.push(`Go Time: ${go}`);
  if (order) parts.push(`Order: ${order}`);
  if (order && minutesTxt) parts.push(`Till Go: ${minutesTxt}`);
...
function isCompletedRecord(record) {
  ...
  return status === "completed" || (shouldDisplayFlag(firstText(record?.goneIn, record?.rs_gone_in)) && hasDisplayResult(record));
}
