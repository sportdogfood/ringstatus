...
const AIRTABLE_WEBHOOK_TIMEOUT_MS = 3500;
...
async function tryRiderLane({ inboundBody, twowayId, phone, env, ctx, addDbg, withDbg }) {
...
function queueAirtablePost(ctx, payload) {
  const work = postToAirtable(payload);
...
async function postToAirtable(payload) {
  const ac = new AbortController();
  const t = setTimeout(() => ac.abort(), AIRTABLE_WEBHOOK_TIMEOUT_MS);
  try {
...
const RINGS = [
  ...
  { ring_number: 6, ring_id: 25, aliases: [] },
  { ring_number: 7, ring_id: 56, aliases: [] },
  { ring_number: 8, ring_id: 57, aliases: [] },
  { ring_number: 9, ring_id: 58, aliases: [] },
  { ring_number: 10, ring_id: 53, aliases: [] },
  { ring_number: 11, ring_id: 22, aliases: [] },
  { ring_number: 12, ring_id: 30, aliases: [] },
  { ring_number: 13, ring_id: 38, aliases: ["south", "south ring", "pista sur"] },
  { ring_number: 14, ring_id: 2, aliases: ["30 ring", "ring 30", "pista 30"] },
  { ring_number: 15, ring_id: 10, aliases: ["derby", "derby field"] },
].map((ring) => ({
  ...ring,
  aliases: Array.from(new Set([
    ...(Array.isArray(ring.aliases) ? ring.aliases : []),
    `ring ${ring.ring_number}`,
    `r${ring.ring_number}`,
    String(ring.ring_number),
  ])),
}));
...
function extractAllRingDigits(norm) {
  ...
  const out = [];
  const pushIfRingNumber = (raw) => {
    const n = Number(raw);
    if (Number.isFinite(n) && n >= 1 && n <= 15) out.push(n);
  };

  let match;
  const explicitRingRx = /\bring\s*(\d{1,2})\b/gi;
  while ((match = explicitRingRx.exec(input)) !== null) {
    pushIfRingNumber(match[1]);
  }

  const explicitShortRx = /\br\s*(\d{1,2})\b/gi;
  while ((match = explicitShortRx.exec(input)) !== null) {
    pushIfRingNumber(match[1]);
  }

  if (/^\d{1,2}$/.test(input)) {
    pushIfRingNumber(input);
  }
