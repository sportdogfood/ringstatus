...
function normalizeAirtableRiderRecord(record) {
  ...
  return {
    ...
    horseName: firstText(f.horseName, f.horse_name, f.barnName, f.barn_name),
...
async function riderLookupResponse(url, env) {
  ...
  let matches = records.filter((record) =>
    record.riderSearch.some((value) => normalize(value) === query.nameNorm)
  );
