...
function tripCategory({ statusCategory: category, minutesToGo, goneIn, hasResult }) {
  if (hasResult || isGoneIn(goneIn) || category === "Completed") return "Completed";
...
function normalizeAirtableRiderRecord(record) {
  ...
  const goneIn = firstText(f.goneIn, f.rs_gone_in);
...
function replyLabelForRecord(record, index, mode) {
  ...
  if (record.tripCategory === "Underway") return "Now";
  if (record.tripCategory === "Completed") return "Completed";
  if (record.tripCategory === "Not Started") return index === 0 ? "Next" : "Not Started";
