...
// Optional:
//   CACHE_TTL       (seconds; default 300; data JSON bypasses this cache)
...
//   AIRTABLE_RIDER_VIEW
// WEC print layout:
//   GET /wec-print-layout?show_no=14906&focus_day=YYYY-MM-DD

const DEFAULT_TTL = 300;
const DEFAULT_AIRTABLE_RIDER_CACHE_TTL = 0;
const DEFAULT_WEC_AIRTABLE_BASE_ID = "app6XS1RvsPNRT6os";
...
async function fetchSglApi(upstreamUrl, req, env) {
  // Prefer service binding when available.
  if (env?.SGL_API && typeof env.SGL_API.fetch === "function") {
    const proxiedReq = new Request(upstreamUrl, req);
    return await env.SGL_API.fetch(proxiedReq);
  }

  // Fallback path: direct upstream fetch using configured auth/cookie headers.
  ...
  if (env?.SGL_TOKEN) headers.set("Authorization", `Bearer ${env.SGL_TOKEN}`);
  headers.set("sgl-request-origin", "SGL-API");
  if (env?.SGL_COOKIE) headers.set("Cookie", String(env.SGL_COOKIE));
  headers.set("Origin", "https://www.wellingtoninternational.com");
  headers.set("Referer", "https://www.wellingtoninternational.com/");

  // Ensure host is derived from upstream URL.
  headers.delete("host");
...
function wecAirtableConfig(env) {
  return {
    token: String(env.AIRTABLE_TOKEN || "").trim(),
    baseId: String(env.WEC_AIRTABLE_BASE_ID || env.AIRTABLE_WEC_SCHEDULES_BASE_ID || DEFAULT_WEC_AIRTABLE_BASE_ID).trim(),
    ringGroupsTable: String(env.AIRTABLE_WEC_RING_GROUPS_TABLE || "ring_groups").trim(),
    printMetaTable: String(env.AIRTABLE_WEC_PRINT_META_TABLE || "wec_print_meta").trim(),
  };
}

function requireWecAirtableConfig(env) {
  const cfg = wecAirtableConfig(env);
  const missing = [];
  if (!cfg.token) missing.push("AIRTABLE_TOKEN");
  if (!cfg.baseId) missing.push("WEC_AIRTABLE_BASE_ID");
...
async function airtableListTable(cfg, table, filterByFormula = "") {
  const records = [];
  let offset = "";

  do {
    const url = new URL(`https://api.airtable.com/v0/${encodeURIComponent(cfg.baseId)}/${encodeURIComponent(table)}`);
    ...
    if (!res.ok) {
      ...
      throw new Error(`Airtable list ${table} failed ${res.status}: ${message}`);
    }
...
function normalizeWecRingGroup(fields) {
  return {
    ring_group_key: firstText(fields.ring_group_key),
    show_no: firstText(fields.show_no),
    focus_day: isoDate(fields.focus_day),
    ring_day_no: firstText(fields.ring_day_no),
    ring_no: firstText(fields.ring_no),
    ring_name: firstText(fields.ring_name),
    source_rows: firstNumber(fields.source_rows) || 0,
    hidden_rows: firstNumber(fields.hidden_rows) || 0,
    visible_classes: firstNumber(fields.visible_classes) || 0,
    visible_rollups: firstNumber(fields.visible_rollups) || 0,
    print_rows: firstNumber(fields.print_rows) || 0,
    portrait_col: firstNumber(fields.portrait_col) || 0,
    landscape_col: firstNumber(fields.landscape_col) || 0,
  };
}

async function wecPrintLayoutResponse(url, env) {
  const cfg = requireWecAirtableConfig(env);
  const showNo = firstText(url.searchParams.get("show_no")) || "14906";
  ...
  const [metaRows, ringRows] = await Promise.all([
    airtableListTable(cfg, cfg.printMetaTable, `{print_meta_key}='${escapeAirtableFormulaString(printMetaKey)}'`),
    airtableListTable(
      cfg,
      cfg.ringGroupsTable,
      `AND({show_no}=${Number(showNo)},IS_SAME({focus_day},DATETIME_PARSE('${escapeAirtableFormulaString(focusDay)}'),'day'))`
    ),
  ]);

  const rings = ringRows
    .map((record) => normalizeWecRingGroup(record.fields || {}))
    .filter((ring) => ring.ring_no)
    .sort((a, b) => a.portrait_col - b.portrait_col || a.print_rows - b.print_rows || a.ring_name.localeCompare(b.ring_name));
...
export default {
  async fetch(req, env, ctx) {
    try {
      ...
      // WEC print-only layout staging from Airtable ring_groups / wec_print_meta.
      if (method === "GET" && path === "/wec-print-layout") {
        return await wecPrintLayoutResponse(url, env);
      }
      ...
      if (path.startsWith("/api/")) {
        ...
        const sglBase = normalizeBaseUrl(env.SGL_API_BASE, "https://sglapi.wellingtoninternational.com");

        const upstreamUrl = new URL(`${sglBase}${forwardPath}`);
        upstreamUrl.search = incomingUrl.search;
        // Force a fresh read path so intermediate caches cannot serve stale API data.
        if (method === "GET" || method === "HEAD") {
          upstreamUrl.searchParams.set("_cb", String(Date.now()));
        }

        const res = await fetchSglApi(upstreamUrl.toString(), req, env);
        const outHeaders = new Headers(res.headers);
        outHeaders.set("Cache-Control", "no-store, no-cache, max-age=0");
