...
//   AIRTABLE_RIDER_CACHE_TTL (seconds; default 20; 0 disables)
//
...
const DEFAULT_AIRTABLE_RIDER_CACHE_TTL = 20;
...
function resultRank(record) {
  ...
  if (normalize(record.status) === "completed") return 1;
  return 9;
}
...
function normalizeAirtableRiderRecord(record) {
  ...
  const sortTime =
    ...
    999999;
  const lastScore = firstText(f.lastScore, f.score1, f.score2, f.score3);
  const lastTime = firstText(f.lastTime);
  const lastPlace = firstText(f.lastPlace, f.lastPlacing, f.latestPlacing);
  ...
  return {
    ...
    minutesToGo: firstNumber(f.minutesToGo, f.rs_min_till_go),
    ...
    goneIn: firstText(f.goneIn, f.rs_gone_in),
    lastPlace,
    lastTime,
    lastScore,
    lastPlacing: lastPlace,
    hasResult: Boolean(lastScore || lastTime || lastPlace),
...
function sortNextRecords(records) {
  return [...records].sort((a, b) => {
    const sr = (a.sortStatus ?? 9) - (b.sortStatus ?? 9);
    if (sr !== 0) return sr;
...
function replyLabelForRecord(record, index, mode) {
  if (mode === "result") return record.hasResult || record.statusCategory === "Completed" ? "Completed" : "Result";
  if (record.statusCategory === "Underway") return "Now";
  if (record.statusCategory === "Completed") return "Completed";
  if (record.statusCategory === "Not Started") return index === 0 ? "Next" : "Not Started";
  return index === 0 ? "Next" : record.statusCategory;
}

function decorateLookupRecords(records, mode) {
  return records.map((record, index) => ({
    ...record,
    replyLabel: replyLabelForRecord(record, index, mode),
  }));
}
...
async function airtableLookupResponse(url, env, lane) {
  ...
  const records = rawRecords.map(normalizeAirtableRiderRecord);
