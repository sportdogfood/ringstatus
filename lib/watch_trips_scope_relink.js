...
function scopeInactiveReason(fields = {}) {
  if (boolValue(fields.archive)) return "archive";
  if (!!strOrNull(fields.dropped_at)) return "dropped";
  if (String(strOrNull(fields.scope_status) || "").toLowerCase() === "dropped") return "dropped";
  if (boolValue(fields.inactive)) return "inactive";
  return null;
}

function classifyWatchTripsHeartbeatRelink(fields = {}, appCtx = {}, heartbeatId) {
  const current = heartbeatLinkIds(fields.heartbeat);
  const inactiveReason = scopeInactiveReason(fields);
  if (inactiveReason) {
    return {
  ...
  return {
    ...
    auto_archive: false,
    deactivate_current_scope: current.length && mismatchedLookup,
  };
}
