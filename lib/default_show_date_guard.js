...
function computeDefaultShowDateGuard({
  ...
  const actualDayCount = showWindowDays !== null && showWindowDays >= 0 ? showWindowDays + 1 : null;
  const expectedDayCount = finiteNumberOrNull(manualDayCount);
  ...
  if (expectedDayCount !== null && actualDayCount !== null && actualDayCount !== expectedDayCount) {
    reasons.push("manual_day_count_mismatch");
  }
  ...
  return {
    ...
    default_show_date_metrics: {
      ...
      actual_day_count: actualDayCount,
      manual_day_count: expectedDayCount,
