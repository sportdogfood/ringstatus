 ...
 *
 * Corrected application order:
 * - Evaluate the FULL sibling set first
 * - Then apply classic / USF overrides INSIDE that set
 * - Do NOT pull classic / USF rows out before grouping
 *
 * Rules:
 * - Forced solo row = is_classic checked OR is_usf checked
 *   - is_parent = false
 *   - is_sibling = false
 *   - parent_class_number = that row's numeric class_number when available, else null
 *   - is_target = true
 *   - entry_sequence:
 *       - 3 when is_classic checked
 *       - 0 when is_usf checked
 *   - targets_last_at = NOW
 *
 ...
 *
 * - Exception:
 *   - If a row is forced solo (is_classic / is_usf), it is still marked solo
...
// Include rows only when they have a valid sibling key and numeric class_number.
// Forced-solo rows remain inside the group for evaluation order, but will be
// marked solo inside the set.
