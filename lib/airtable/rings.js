  ...
  if (groups.length > 0) {
    ...
    if (!isActive) {
      ...
      if (Number.isFinite(active_actual_start_epoch_ms) && done > 0 && total && total > 0) {
        ...
        mins_per_trip = elapsed_min != null ? (elapsed_min / done) : null;
...
for (const req of uniqueRingRequests.values()) {
  ...
  try {
    ...
    log(`${tag} COMPUTE_OK status=${computed.status} offset=${computed.ring_offset_min} ring_name=${computed.ring_name || ""}`);
