 ...
 *  - app_dt  = heartbeat.app_sql_date when available, else trigger app_dt
 *  - default-day resolution is expected to be computed upstream and stored on heartbeat
...
async function fetchResolvedHeartbeatScope(showIdNum) {
  if (!Number.isFinite(showIdNum) || showIdNum <= 0) return null;

  const heartbeatTable = safeGetTable("heartbeat");
  if (!heartbeatTable) return null;
  ...
  const heartbeatFields = [
    ...
    "schedule_endpoint",
    "hb_at",
  ].filter((fieldName) => tableHasField(heartbeatTable, fieldName));

  const queryOptions = { fields: heartbeatFields };
  if (tableHasField(heartbeatTable, "hb_at")) {
    queryOptions.sorts = [{ field: "hb_at", direction: "desc" }];
  }

  const heartbeatQuery = await heartbeatTable.selectRecordsAsync(queryOptions);

  let best = null;
  let bestMs = -1;

  for (const rec of heartbeatQuery.records) {
    const heartbeatShowId = toFiniteNumberOrNull(
      getCellValueFirst(rec, ["app_show_id", "show_id"])
    );
    if (heartbeatShowId !== showIdNum) continue;

    const heartbeatAppSqlDate = toIsoDateOnly(
      getCellValueFirst(rec, ["app_sql_date", "default_app_sql_date_is"])
    );
    if (!heartbeatAppSqlDate) continue;

    const candidate = {
      record_id: rec.id,
      app_sql_date: heartbeatAppSqlDate,
      set_to_default_app_sql_date: boolValue(getCellValueSafe(rec, "set_to_default_app_sql_date")),
      default_app_sql_date_is: toIsoDateOnly(getCellValueSafe(rec, "default_app_sql_date_is")) || "",
      app_sql_date_source: normalizeText(getCellValueSafe(rec, "app_sql_date_source")) || "",
      schedule_endpoint: normalizeText(getCellValueSafe(rec, "schedule_endpoint")) || "",
      hb_at: normalizeText(getCellValueSafe(rec, "hb_at")) || "",
    };

    const candidateMs = parseRunTimeMs(candidate.hb_at);
...
let appDtText = String(appDtRaw ?? "").trim();
let appDtSource = "trigger_app_dt";
let heartbeatScope = null;

// Heartbeat carries the resolved app date, including default-day overrides.
if (Number.isFinite(appSidNum) && appSidNum > 0) {
  try {
    heartbeatScope = await fetchResolvedHeartbeatScope(appSidNum);
    if (heartbeatScope?.app_sql_date) {
      appDtText = heartbeatScope.app_sql_date;
      appDtSource = heartbeatScope.app_sql_date_source || "heartbeat_app_sql_date";
    }
  } catch (err) {
    await logAutomationErr({
      error_type: "heartbeat_scope_lookup_failed",
...
output.set("app_dt_source", appDtSource);
...
output.set("heartbeat_scope_record_id", heartbeatScope?.record_id || "");
output.set("heartbeat_app_sql_date", heartbeatScope?.app_sql_date || "");
output.set("heartbeat_set_to_default_app_sql_date", !!heartbeatScope?.set_to_default_app_sql_date);
output.set("heartbeat_default_app_sql_date_is", heartbeatScope?.default_app_sql_date_is || "");
output.set("heartbeat_app_sql_date_source", heartbeatScope?.app_sql_date_source || "");
output.set("heartbeat_schedule_endpoint", heartbeatScope?.schedule_endpoint || "");
