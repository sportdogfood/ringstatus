...
function existingFieldNames(table, candidates = []) {
  return (Array.isArray(candidates) ? candidates : []).filter(name => name && tableHasField(table, name));
}
...
function getScopeSpec(table, showRecordId = null) {
  return {
    sidFields: existingFieldNames(table, ["app_sid", "show_id", "sid"]),
    dtFields: existingFieldNames(table, ["app_dt", "show_date", "app_sql_date", "date", "dt"]),
    showsField: !!showRecordId && tableHasField(table, "shows") ? "shows" : null,
  };
}
...
function getScopeFieldsToLoad(keyField, scopeSpec, extraFields = []) {
  ...
  for (const fieldName of (Array.isArray(scopeSpec?.sidFields) ? scopeSpec.sidFields : [])) {
    if (!out.includes(fieldName)) out.push(fieldName);
  }
  for (const fieldName of (Array.isArray(scopeSpec?.dtFields) ? scopeSpec.dtFields : [])) {
    if (!out.includes(fieldName)) out.push(fieldName);
  }
  if (scopeSpec?.showsField && !out.includes(scopeSpec.showsField)) out.push(scopeSpec.showsField);
...
function recordMatchesScope(rec, scopeSpec, scopeAppSid, scopeAppDt, showRecordId = null) {
  ...
  if (scopeSpec?.showsField && showRecordId) {
    sawScopeField = true;
    const showIds = linkedRecordIds(getCellValueSafe(rec, scopeSpec.showsField));
    if (showIds.includes(showRecordId)) return true;
  }

  const sidFields = Array.isArray(scopeSpec?.sidFields) ? scopeSpec.sidFields : [];
  const dtFields = Array.isArray(scopeSpec?.dtFields) ? scopeSpec.dtFields : [];

  if (sidFields.length || dtFields.length) {
    sawScopeField = true;
    const sidMatch = sidFields.length
      ? sidFields.some(fieldName => sameScopeSid(getCellValueSafe(rec, fieldName), scopeAppSid))
      : true;
    const dtMatch = dtFields.length
      ? dtFields.some(fieldName => sameScopeDt(getCellValueSafe(rec, fieldName), scopeAppDt))
      : true;
...
function getPeopleTripsFromPayload(payload) {
...
async function upsertByKeyScoped(table, keyField, rows, writableSet, scopeAppSid, scopeAppDt, tableNameForErr, scopeShowRecordId = null) {
  ...
  try {
    ...
    diag.scope = {
      sid_fields: scopeSpec.sidFields,
      dt_fields: scopeSpec.dtFields,
      shows_field: scopeSpec.showsField || "",
...
function buildClassRowFromWatchScheduleRecord(rec) {
  const classIdNum = toFiniteNumberOrNull(getCellValueFirst(rec, ["class_id", "classes_id"]));
  if (classIdNum === null) return null;

  const classGroupId = toFiniteNumberOrNull(getCellValueFirst(rec, ["class_group_id", "group_id"]));
  const ringNumber = toFiniteNumberOrNull(getCellValueFirst(rec, ["ring_number", "ring"]));
  const estimatedStartTime = normalizeText(
    getCellValueFirst(rec, ["estimated_start_time", "latestStart", "start_time_default", "start_time"])
  );

  return {
    class_id: Number(classIdNum),
    class_number: normalizeEntryNumber(getCellValueFirst(rec, ["class_number", "number"])),
    class_name: normalizeText(getCellValueFirst(rec, ["class_name", "name"])),
    schedule_sequencetype: normalizeText(getCellValueFirst(rec, ["schedule_sequencetype", "sequence_type", "sequencetype"])),
    class_type: normalizeText(getCellValueFirst(rec, ["class_type", "type"])),
    class_group_id: classGroupId === null ? undefined : Number(classGroupId),
    group_name: normalizeText(getCellValueFirst(rec, ["group_name", "class_group_name"])),
    class_groupxclasses_id: toFiniteNumberOrNull(getCellValueFirst(rec, ["class_groupxclasses_id", "groupxclasses_id"])) ?? undefined,
    ...
    estimated_end_time: normalizeText(getCellValueFirst(rec, ["estimated_end_time", "estimatedEndTime"])) || undefined,
    class_group_sequence: toFiniteNumberOrNull(getCellValueFirst(rec, ["class_group_sequence", "group_sequence"])) ?? undefined,
  };
}
...
const watchScheduleClassIdSample = [];
let watchScheduleRowsWithBlankClassId = 0;
...
if (!watchScheduleTable) {
  ...
  try {
    ...
    for (const rec of watchScheduleQuery.records) {
      ...
      if (!row) {
        watchScheduleRowsWithBlankClassId += 1;
        continue;
      }

      const key = String(row.class_id);
      pushSample(watchScheduleClassIdSample, key, 10);
...
for (const primaryPid of pids) {
...

  {
    ...
    const trips = getPeopleTripsFromPayload(payload);
...
for (const pt of peopleTrips) {
  ...
  const s = watchScheduleByClassId.get(classIdKey);
  if (!s) {
    peopleTripsMissingSchedule += 1;
    pushSample(peopleTripsMissingScheduleSample, `${pt.class_id}|${pt.entryxclasses_uuid}`);
    continue;
