...
function isFetchQuotaErr(err) {
  return String(err?.code || "").trim() === "fetch_quota_guard";
}
...
// SCHEDULE FLATTEN
//////////////////////
function flattenSchedulePayload(payload) {
  const classRows = [];
  ...
  for (const ring of rings) {
    const ring_number_raw = ring?.ring_number ?? ring?.ring ?? ring?.ringNumber ?? null;
    const ring_number = toFiniteNumberOrNull(ring_number_raw);
    const ring_name   = String(ring?.ring_name ?? ring?.name ?? "");

    const classes = Array.isArray(ring?.classes) ? ring.classes : [];
    for (const c of classes) {
      const class_group_id = toFiniteNumberOrNull(c?.class_group_id ?? c?.group_id);
      if (class_group_id !== null) {
        const groupKey = String(class_group_id);
        if (!groupById.has(groupKey)) groupOrder.push(groupKey);

        const prev = groupById.get(groupKey) || {};
        groupById.set(groupKey, {
          class_group_id: Number(class_group_id),
          group_name: String(c?.group_name ?? c?.class_group_name ?? prev.group_name ?? ""),
          ring_number: ring_number === null ? null : Number(ring_number),
          ring_name: ring_name || String(prev.ring_name || ""),
          estimated_start_time: String(c?.estimated_start_time ?? c?.start_time ?? prev.estimated_start_time ?? ""),
          class_group_sequence: toFiniteNumberOrNull(c?.group_sequence ?? c?.class_group_sequence) ?? prev.class_group_sequence ?? null,
        });
      }

      const class_id = toFiniteNumberOrNull(c?.class_id ?? c?.id);
      ...
      classRows.push({
        ...
        class_group_id: class_group_id === null ? null : Number(class_group_id),
        ...
        class_group_sequence: toFiniteNumberOrNull(c?.group_sequence ?? c?.class_group_sequence),
      });
    }
  }
  ...
  for (const groupKey of groupOrder) {
    const row = { ...(groupById.get(groupKey) || {}) };
    if (!hasValue(row.class_group_sequence)) row.class_group_sequence = ++seq;
    groupRows.push(row);
  }

  const groupSeqMap = new Map(
    groupRows
      .filter(r => hasValue(r?.class_group_id))
      .map(r => [String(r.class_group_id), r])
  );

  for (const r of classRows) {
    if (hasValue(r.class_group_sequence)) continue;
    const groupRow = r.class_group_id ? groupSeqMap.get(String(r.class_group_id)) : null;
    if (groupRow && hasValue(groupRow.class_group_sequence)) {
      r.class_group_sequence = Number(groupRow.class_group_sequence);
    }
  }

  return { classRows, groupRows };
}
...
try {
  ...
  const flattenedSchedule = flattenSchedulePayload(schedulePayload);
  schedRows = flattenedSchedule.classRows;
  schedGroupRows = flattenedSchedule.groupRows;
} catch (err) {
  await logAutomationErr({
...
const validDailyClassIdSet = watchScheduleByClassId.size > 0
  ? new Set([...watchScheduleByClassId.keys()])
  : new Set([...classById.keys()]);
const validDailyClassSource = watchScheduleByClassId.size > 0 ? "watch_schedule" : "schedule_endpoint";
...
const classPayloadCache = new Map();

async function getClassPayload(classId) {
  const classIdTok = normalizeKeyStr(classId);
  if (!classIdTok) return null;
  if (classPayloadCache.has(classIdTok)) return classPayloadCache.get(classIdTok);

  const classUrl = `${base_url}/classes/${encodeURIComponent(classIdTok)}/?show_id=${encodeURIComponent(appSidNum)}&customer_id=${encodeURIComponent(customer_id)}`;
  const payload = await fetchJson(classUrl);
  classPayloadCache.set(classIdTok, payload);
  return payload;
}

function buildClassRowFromClassPayload(classId, payload) {
  const cls = payload?.class || {};
  const related = payload?.class_related_data || {};

  const classIdNum = toFiniteNumberOrNull(cls?.class_id ?? classId);
  if (classIdNum === null) return null;

  const classGroupId = toFiniteNumberOrNull(related?.class_group_id ?? cls?.class_group_id);
  const groupRow = classGroupId === null ? null : groupScheduleById.get(String(classGroupId));

  const ringNum = groupRow?.ring_number ?? toFiniteNumberOrNull(related?.ring);
  const estimatedStartTime =
    normalizeText(groupRow?.estimated_start_time) ||
    normalizeText(related?.estimated_time) ||
    normalizeText(related?.actual_time) ||
    normalizeText(related?.default_time);

  return {
    class_id: Number(classIdNum),
    class_number: normalizeEntryNumber(cls?.number ?? cls?.class_number),
    class_name: normalizeText(cls?.name ?? cls?.class_name),
    schedule_sequencetype: normalizeText(cls?.schedule_sequencetype ?? related?.schedule_sequencetype),
    class_type: normalizeText(cls?.class_type ?? related?.type),
    class_group_id: classGroupId === null ? undefined : Number(classGroupId),
    group_name: normalizeText(groupRow?.group_name ?? related?.group_name),
    class_groupxclasses_id: toFiniteNumberOrNull(related?.class_groupxclasses_id ?? cls?.class_groupxclasses_id) ?? undefined,
    ring_number: ringNum === null ? undefined : Number(ringNum),
    ring_name: normalizeText(groupRow?.ring_name ?? related?.ring_name),
    estimated_start_time: estimatedStartTime || "",
    class_group_sequence: hasValue(groupRow?.class_group_sequence) ? Number(groupRow.class_group_sequence) : undefined,
  };
}
...
const suppressEmptyPayloadPlaceholders = scheduleFetchOk && !scheduleHasRows;
...
const forceClearInScopeBecauseNoSchedule = scheduleFetchOk && !scheduleHasRows;
...
//////////////////////
const uniqueClassIdsRaw = [...new Set(
  watchTripsRows
...
const uniqueClassIds = [
  ...uniqueClassIdsRaw.filter(classIdStr => !classById.has(classIdStr)),
  ...uniqueClassIdsRaw.filter(classIdStr => classById.has(classIdStr)),
];

const enrichErrors = [];
const enrichUpdates = [];
let classEnrichSkippedDueToFetchQuota = 0;
const classEnrichSkippedDueToFetchQuotaSample = [];

async function enrichOneClass(classIdStr) {
  const classIdTok = normalizeKeyStr(classIdStr);
  if (!classIdTok) return;
  if (fetchQuotaExceeded && !classPayloadCache.has(classIdTok)) {
    classEnrichSkippedDueToFetchQuota += 1;
    pushSample(classEnrichSkippedDueToFetchQuotaSample, classIdTok);
    return;
  }

  try {
    const payload = await getClassPayload(classIdStr);

    const classMeta = buildClassRowFromClassPayload(classIdStr, payload) || {};
    const related = payload?.class_related_data || {};
    const endKey = groupEndKey(classMeta?.ring_number, classMeta?.class_group_id);
    const estimated_end_time = endKey
      ? (groupEndTime.get(endKey) || classMeta?.estimated_start_time || undefined)
      : (classMeta?.estimated_start_time || undefined);
    const classFields = {
      ...
      class_number: classMeta?.class_number,
      class_name: classMeta?.class_name || undefined,
      schedule_sequencetype: classMeta?.schedule_sequencetype || undefined,
      class_type: classMeta?.class_type || undefined,
      class_group_id: hasValue(classMeta?.class_group_id) ? Number(classMeta.class_group_id) : undefined,
      group_name: classMeta?.group_name || undefined,
      class_groupxclasses_id: hasValue(classMeta?.class_groupxclasses_id) ? Number(classMeta.class_groupxclasses_id) : undefined,
      ring_number: hasValue(classMeta?.ring_number) ? Number(classMeta.ring_number) : undefined,
      estimated_start_time: classMeta?.estimated_start_time || undefined,
      estimated_end_time: estimated_end_time,
      class_group_sequence: hasValue(classMeta?.class_group_sequence) ? Number(classMeta.class_group_sequence) : undefined,
    };
...
if (hasRealTripsPayload || forceClearInScopeBecauseNoSchedule) {
...
} else if (scheduleFetchOk && !hasRealTripsPayload) {
