...
async function logAutomationErr({
  ...
  try {
    ...
    const W_ERRS = getWritableFieldSet(automationErrsTable);
    const safeErrorType = String(error_type || "runtime_error").trim() || "runtime_error";
    const safePid = String(pid ?? "").trim();
    const safeAppShowId = String(app_show_id ?? "").trim();
    const safePeopleShowId = String(people_show_id ?? "").trim();
    const safeKey =
      String(automation_key || `${AUTOMATION_NAME}|${safePid || safeAppShowId || "na"}|${RUN_ID}|${safeErrorType}`)
        .trim();

    const row = pickWritable(W_ERRS, {
      automation_key: safeKey,
      automation_name: AUTOMATION_NAME,
      error_type: safeErrorType,
      app_sql_date: APP_DT_FOR_ERRS,
      run_id: RUN_ID,
      last_run: APP_DT_FOR_ERRS,
      message: String(message || "").trim() || safeErrorType,
      pid: safePid || undefined,
      app_show_id: safeAppShowId || undefined,
      people_show_id: safePeopleShowId || undefined,
    });
  ...
  try {
    ...
    if (trips.length === 0) {
      await logAutomationErr({
        ...
        message: `Skipped pid ${pidTok}: empty trips payload.`,
        pid: pidTok,
        app_show_id: appSidNum,
...
//////////////////////
const uniqueClassIds = [...new Set(watchTripsRows.map(r => String(r.class_id)).filter(Boolean))];
