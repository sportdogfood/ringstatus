...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // STATE 1: no measurable schedule data
      if (!groups.length) {
...
const RINGS = [
  { ring_number: 1, ring_id: 51, aliases: ["intl", "international", "r1", "ring 1"] },
  { ring_number: 2, ring_id: 9, aliases: ["grand", "mische", "r2", "ring 2"] },
  { ring_number: 3, ring_id: 37, aliases: ["rost", "r3", "ring 3"] },
  { ring_number: 4, ring_id: 52, aliases: ["mogavero", "mogav", "r4", "ring 4"] },
  { ring_number: 5, ring_id: 49, aliases: ["denemethy", "denem", "r5", "ring 5"] },
  ...
  { ring_number: 13, ring_id: 38, aliases: ["south", "south ring", "r13", "ring 13", "pista sur"] },
  { ring_number: 14, ring_id: 2, aliases: ["30 ring", "30 dollar", "pista 30", "r14","ring 14"] },
...
function buildPreStartReply({ groups, ringName, asOf }) {
...
function buildInProgressReply({ groups, runningIndex, ringName, asOf, anchorMs, debugAdd }) {
  ...
  const eeiMin = chosenCurMs ? Math.max(0, Math.round((curLeft * chosenCurMs) / 60000)) : null;
  const esiMin = nextGroup
