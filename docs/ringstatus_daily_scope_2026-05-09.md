...
- `mode_control = IDLE`: throttle heavy work without fully stopping the heartbeat record.
...
Manual time override: `manual_time_override` is the row-level stop switch for schedule/trip time writes. On `watch_schedule`, when checked, `schedules_dailyv2.js` and `schedules_calculatorv2.js` must not patch `estimated_start_time` for that row on later cycles. On `watch_trips`, when checked, `trips_dailyv2.js` and `trips_tagger.js` must not patch trip timing fields such as `estimated_start_time`, `estimated_end_time`, `estimated_go_time`, `estimated_time`, or `actual_time`. The code also accepts the misspelled `manual_time_overide` alias defensively, but Airtable metadata confirms the real field is `manual_time_override`. This is the preferred way to protect a known manual correction without creating a separate override table or writing sentinel text such as `"check gate"`.
