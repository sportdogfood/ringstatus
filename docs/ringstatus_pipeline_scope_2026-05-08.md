...
| Version | Date | Change |
| --- | --- | --- |
| v2026.05.18.1 | 2026-05-18 | Updated the focused `show` contract: active runs may include zero, one, or multiple heartbeat-checked `show` rows. Zero active rows must still write heartbeat with `scope_status = no-active-feeds` and skip downstream heavy lanes. Multiple active rows run one scoped pass per `show` record. Minimum manual fields are `show_id`, `customer_id`, and `focus_day`; `start_date`, `end_date`, and `show_name` may be enriched by tagger. Added `show.manual_day_count` as a manually owned inclusive day-count check against `start_date..end_date`. |
...
Bad-time overwrite guard: in shifted `NIGHT` pre-live schedule runs, values outside `07:00:00` through `19:00:00` are treated as suspicious when they come from schedule/manual/HTML sources. This mirrors the prior Airtable validation logic that would display `check gate`, but the writer must not store `"check gate"` in `estimated_start_time`. The writer should preserve an existing nonblank Airtable `estimated_start_time` when the incoming suspicious value differs, and should omit suspicious values for new rows or rows that already hold the same suspicious value. The run summary must report guard counts and samples. This keeps manual corrections from being overwritten on the next cycle without adding a new override table or changing live-day enrichment rules.

Manual time override rule: `manual_time_override` is a per-row lock on both `watch_schedule` and `watch_trips`. If checked on `watch_schedule`, schedule writers must not patch `estimated_start_time` for that row, even if fresh schedule JSON, cached JSON, manual JSON, manual HTML, or `groups_live` supplies a value. If checked on `watch_trips`, trip writers must not patch timing fields such as `estimated_start_time`, `estimated_end_time`, `estimated_go_time`, `estimated_time`, or `actual_time`. This protects intentional manual corrections while keeping all other schedule/trip fields eligible for normal refresh. The misspelled `manual_time_overide` may be accepted only as a defensive alias; it is not the documented Airtable field name.
...
Every `getLiveClassData` ping attempt must be written to `automation_errs` by `trips_tagger.js`. This includes successful payloads, because the table is also the endpoint-attempt history. Successful pings should use `automation_name = trips_tagger_getLiveClassData`, `error_type = liveclass_payload_ok`, `resolved = true`, and include the endpoint path, `show_id`, `cid`, `cgid`, HTTP status, body length, payload class id, and row count in `message`. Empty, invalid, HTTP-failed, or wrong-class payloads should write the same automation name with `resolved = false` and a liveclass-specific failure type.
