...
| Version | Date | Change |
| --- | --- | --- |
| v2026.05.09.8 | 2026-05-09 | Added `manual_time_overide` as the row-level stop switch for time writes on both `watch_schedule` and `watch_trips`: when checked, schedule/trip writers must omit time fields from future patches for that row so manual corrections are not repopulated by the next cycle. |
...
Manual time override rule: `manual_time_overide` is a per-row lock on both `watch_schedule` and `watch_trips`. If checked on `watch_schedule`, schedule writers must not patch `estimated_start_time` for that row, even if fresh schedule JSON, cached JSON, manual JSON, or manual HTML supplies a value. If checked on `watch_trips`, trip writers must not patch timing fields such as `estimated_start_time`, `estimated_end_time`, `estimated_go_time`, `estimated_time`, or `actual_time`. This protects intentional manual corrections while keeping all other schedule/trip fields eligible for normal refresh.
